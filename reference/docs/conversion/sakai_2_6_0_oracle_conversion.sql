...
MERGE INTO SAKAI_REALM_RL_FN FN USING(select (select REALM_KEY from SAKAI_REALM where REALM_ID = '!group.template.portfolio') REALM_KEY, (select ROLE_KEY from SAKAI_REALM_ROLE where ROLE_NAME = 'CIG Coordinator') ROLE_KEY, (select FUNCTION_KEY from SAKAI_REALM_FUNCTION where FUNCTION_NAME = 'content.all.groups') FUNCTION_KEY FROM DUAL) RRF
ON (FN.REALM_KEY = RRF.REALM_KEY AND FN.ROLE_KEY = RRF.ROLE_KEY AND FN.FUNCTION_KEY = RRF.FUNCTION_KEY)
WHEN NOT MATCHED THEN INSERT (FN.REALM_KEY, FN.ROLE_KEY, FN.FUNCTION_KEY) VALUES (RRF.REALM_KEY, RRF.ROLE_KEY, RRF.FUNCTION_KEY);

MERGE INTO SAKAI_REALM_RL_FN FN USING(select (select REALM_KEY from SAKAI_REALM where REALM_ID = '!group.template.portfolio') REALM_KEY, (select ROLE_KEY from SAKAI_REALM_ROLE where ROLE_NAME = 'CIG Coordinator') ROLE_KEY, (select FUNCTION_KEY from SAKAI_REALM_FUNCTION where FUNCTION_NAME = 'content.hidden') FUNCTION_KEY FROM DUAL) RRF
ON (FN.REALM_KEY = RRF.REALM_KEY AND FN.ROLE_KEY = RRF.ROLE_KEY AND FN.FUNCTION_KEY = RRF.FUNCTION_KEY)
WHEN NOT MATCHED THEN INSERT (FN.REALM_KEY, FN.ROLE_KEY, FN.FUNCTION_KEY) VALUES (RRF.REALM_KEY, RRF.ROLE_KEY, RRF.FUNCTION_KEY);

MERGE INTO SAKAI_REALM_RL_FN FN USING(select (select REALM_KEY from SAKAI_REALM where REALM_ID = '!site.template.portfolio') REALM_KEY, (select ROLE_KEY from SAKAI_REALM_ROLE where ROLE_NAME = 'CIG Coordinator') ROLE_KEY, (select FUNCTION_KEY from SAKAI_REALM_FUNCTION where FUNCTION_NAME = 'content.all.groups') FUNCTION_KEY FROM DUAL) RRF
ON (FN.REALM_KEY = RRF.REALM_KEY AND FN.ROLE_KEY = RRF.ROLE_KEY AND FN.FUNCTION_KEY = RRF.FUNCTION_KEY)
WHEN NOT MATCHED THEN INSERT (FN.REALM_KEY, FN.ROLE_KEY, FN.FUNCTION_KEY) VALUES (RRF.REALM_KEY, RRF.ROLE_KEY, RRF.FUNCTION_KEY);
