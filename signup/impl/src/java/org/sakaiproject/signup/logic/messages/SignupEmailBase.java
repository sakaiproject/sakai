...
abstract public class SignupEmailBase implements SignupEmailNotification, MeetingTypes {
	...
	protected String getServerFromAddress() {
		return getSakaiFacade().getServerConfigurationService().getString("smtpFrom@org.sakaiproject.email.api.EmailService",rb.getString("noReply@") + getSakaiFacade().getServerConfigurationService().getServerName());
	}
