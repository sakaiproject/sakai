...
public class HtmlCheckboxRendererBase extends HtmlRenderer
{
            ...
            UISelectMany selectMany) throws IOException
    {
        ...
        Set lookupSet = org.apache.myfaces.shared_tomahawk.renderkit.RendererUtils.getSubmittedValuesAsSet(
                facesContext, selectMany, converter, selectMany);
        boolean useSubmittedValues = lookupSet != null;

        if (!useSubmittedValues)
        {
            lookupSet = org.apache.myfaces.shared_tomahawk.renderkit.RendererUtils.getSelectedValuesAsSet(
                    facesContext, selectMany, converter, selectMany);
        }

        int itemNum = 0;

        for (Iterator it = org.apache.myfaces.shared_tomahawk.renderkit.RendererUtils.getSelectItemList(
            ...
            boolean renderId, Integer itemNum) throws IOException
    {
        ...
        if (checked)
        {
            writer.writeAttribute(HTML.CHECKED_ATTR, 
                    org.apache.myfaces.shared_tomahawk.renderkit.html.HTML.CHECKED_ATTR, null);
        }
