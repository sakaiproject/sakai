...
public class AutoSubmitAssignmentsJob implements Job {
    ...
    private int autoSubmitDraftSubmissions() {
        ...
        try {
            ...
            while (true) {
                batch = assignmentService.getAllEligibleDraftSubmissions(batchSize, 0);
                ...
                if (successfulInBatch == 0) {
                    log.warn("No successful auto-submits in batch {}; stopping to avoid reprocessing loop", batchNumber - 1);
                    break;
                }
            }
            ...
            synchronized (this) {
                if (!emailTemplateRegistered) {
                    try {
                        ...
                        try (java.io.InputStream templateStream =
                                     cl.getResourceAsStream("templates/" + AssignmentConstants.EMAIL_TEMPLATE_AUTO_SUBMIT_ERRORS_FILE_NAME)) {
