...
public class AssignmentRepositoryImpl extends BasicSerializableRepository<Assignment, String> implements AssignmentRepository {
    ...
    public List<Assignment> findAutoSubmitAssignmentsBySite(String siteId, java.time.Instant now) {
        ...
        return assignments.stream()
                .filter(a -> "true".equals(
                Optional.ofNullable(a.getProperties())
                        .map(p -> p.get(AssignmentConstants.ASSIGNMENT_AUTO_SUBMIT_ENABLED))
                        .orElse(null)))
                .collect(Collectors.toList());
    }
    ...
    public List<AssignmentSubmission> findAllEligibleDraftSubmissions(int limit, int offset) {
        ...
        List<AssignmentSubmission> submissions = geCurrentSession().createCriteria(AssignmentSubmission.class)
                ...
                .add(Restrictions.isNull("dateSubmitted")) // Not submitted
                .add(Restrictions.eq("userSubmission", Boolean.TRUE)) // Saved submissions only
