...
public class AssignmentActionTest {
    ...
    @Test
    public void testPublishAssignmentDelegatesToAssignmentService() throws Exception {
        SessionState state = new SessionStateFake();
        Assignment assignment = Mockito.mock(Assignment.class);
        Map<String, String> properties = new HashMap<>();

        Mockito.when(assignment.getProperties()).thenReturn(properties);
        Mockito.when(assignment.getDraft()).thenReturn(Boolean.FALSE);

        assignmentAction.publishAssignment(state, "site-id", assignment);

        Mockito.verify(assignmentService).publishAssignment(assignment);
    }
}
