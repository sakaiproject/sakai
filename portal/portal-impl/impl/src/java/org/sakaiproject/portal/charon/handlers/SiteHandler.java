...
public class SiteHandler extends WorksiteHandler {
	...
	@Autowired private ProfileImageLogic profileImageLogic;
		...
		if ((siteId != null) && (parts.length == 5) && (parts[3].equals("page-reset")))
		{
			...
			if (page != null)
			{
                ...
                for (ToolConfiguration tool : page.getTools()) {
                    ToolSession ts = s.getToolSession(tool.getId());
                    ts.clearAttributes();
                    if (portal.isPortletPlacement(tool)) hasJSR168 = true;
                }
			}
	...
	public void doSite(HttpServletRequest req, HttpServletResponse res, Session session,
		...
		if (siteId == null)
		{
			...
			else
			{
				...
				AllSitesViewImpl allSites = (AllSitesViewImpl) portal.getSiteHelper().getSitesView(SiteView.View.ALL_SITES_VIEW, req, session, null);
		...
		if ( toolId != null ) {
			...
			if ( siteTool != null && parts.length >= 5 ) {
				...
				allowBuffer = allowBufferContent(site, siteTool);
				...
				if ( allowBuffer ) {
					...
					if ( matched ) {
						...
						ActiveTool tool = toolManager.getActiveTool(commonToolId);
		...
		if (userId != null) {
			...
			final ResourceProperties siteNavProps = readOnlyPrefs.getProperties(PreferencesService.SITENAV_PREFS_KEY);
			...
			if (!StringUtils.equals(currentExpanded, "true") || !StringUtils.equals(expandedSite, siteId)) {
				...
				try {
					...
					ResourcePropertiesEdit props = prefs.getPropertiesEdit(PreferencesService.SITENAV_PREFS_KEY);
					...
					if (!themeEnabled) {
						prefs.getPropertiesEdit(PreferencesService.USER_SELECTED_UI_THEME_PREFS).addProperty("theme", "sakaiUserTheme-notSet");
					}
				} catch (Exception any) {
		...
		if ( BC instanceof Map ) {
			if ( req.getMethod().equals("POST") ) {
				...
				String queryString = req.getQueryString();
				if ( queryString != null ) queryUrl.append('?').append(queryString);
				log.debug("It is tacky to return markup on a POST CTI={} URL={}", commonToolId, queryUrl);
			}
		...
		rcontext.put("activePageId", page != null ? page.getId() : null);
		...
		if (presenceEvents) presenceService.setPresence(siteId + PresenceService.PRESENCE_SUFFIX);
	 ...
	 * @throws IOException
	 */
	protected void doSendResponse(PortalRenderContext rcontext, HttpServletResponse res) throws IOException {
	...
	protected void includeSiteNav(PortalRenderContext rcontext, HttpServletRequest req, Session session, String siteId, String toolId)
	{
		if (rcontext.uses(INCLUDE_SITE_NAV))
		{
			...
			rcontext.put("siteNavHasAccessibilityURL", accessibilityURL != null && !accessibilityURL.isEmpty());
			rcontext.put("siteNavAccessibilityURL", accessibilityURL);
			rcontext.put("siteNavTopLogin", topLogin);
			rcontext.put("siteNavLoggedIn", loggedIn);
		...
		if (rcontext.uses(INCLUDE_TABS))
		{
			...
			else if (allowroleswap)
			{
				...
				if (activeSite.getType() != null && roleInSite) {
					...
					// if the userRole is null, this means they are more than likely a Delegated Access user.
					// Since the security check has already allowed
			...
			rcontext.put("viewAsStudentLink", roleswapcheck); // this will tell our UI if we want the link for swapping roles to display
			...
			if (loggedIn) 
			{
				...
				try {
					...
					log.debug("no property named tab:label, {}", e.toString());
				}

				try {
					sidebarCollapsed = props.getBooleanProperty(PortalConstants.PROP_SIDEBAR_COLLAPSED);
                } catch (Exception e) {
					log.warn("Exception caught whilst getting sidebarCollapsed, {}", e.toString());
				}
				...
				try {
					...
					log.warn("Exception caught whilst getting currentExpanded, {}", e.toString());
				}
				...
				try {
					...
					log.warn("Exception caught whilst getting toolMaximised, {}", e.toString());
				}
			}

			rcontext.put("tabDisplayLabel", tabDisplayLabel);
			rcontext.put(PortalConstants.PROP_SIDEBAR_COLLAPSED, sidebarCollapsed);
			if (expandedSite.equals(siteId)) {
				rcontext.put(PortalConstants.PROP_CURRENT_EXPANDED, currentExpanded);
			}
			rcontext.put("toolMaximised", toolMaximised);
			...
			rcontext.put("tabsAddLogout", addLogout);
			if (addLogout)
			{
	...
	public boolean checkBufferBypass(HttpServletRequest req, ToolConfiguration siteTool)
	{
		...
		if (Boolean.parseBoolean(req.getHeader("wicket-ajax"))) {
			matched = true;
		}
		return matched;
	}
	 ...
	 */
	public boolean allowBufferContent(Site site, ToolConfiguration siteTool) {
		...
		if (tidAllow.contains(":none:")) return false;
			...
			ToolConfiguration siteTool)
	{
		...
		try {
			...
			portal.includeTool(res, req, siteTool, true);
		...
		if (responseStr == null || responseStr.isEmpty()) return Boolean.FALSE;

		PageParts pp = parseHtmlParts(responseStr);
		if (pp != null)
		{
			if (debug)
			{
	 ...
	 */
	PageParts parseHtmlParts(String responseStr) {
		...
		log.debug("Frameless HS={} HE={} BS={} BE={}", headStart, headEnd, bodyStart, bodyEnd);
			...
			String toolPathInfo) throws ToolException, IOException
	{
		...
		ActiveTool tool = toolManager.getActiveTool(siteTool.getToolId());
		if (tool == null)
		{
			return false;
		}
		...
		log.debug("doToolBuffer siteTool={} TCP={} TPI={}", siteTool, toolContextPath, toolPathInfo);
