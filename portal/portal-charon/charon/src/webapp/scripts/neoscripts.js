...
var dhtml_view_sites = function(){
    ...
    dhtml_view_sites = function(){
        if ($PBJQ('#selectSite').css('display') == 'none') {
            $PBJQ('div#selectSite div').show();
            $PBJQ('div#selectSite').slideDown('fast', function(){
                // check if $PBJQ('#otherSiteList li').length > some number, then show search
                // otherwise not
                   if($PBJQ('div#otherSitesCategorWrap').height() > 300){
                    $PBJQ('div#otherSitesCategorWrap').height(300).css({overflow:"auto"});
                }
            ...
            if(txtSearch.length) {
                $PBJQ(txtSearch[0]).keydown(function (e) {
                    if(e.keyCode ===27) {
                        $PBJQ('div#selectSite div').hide();
                        $PBJQ('div#selectSite').slideUp('fast'); // hide the box
                        removeDHTMLMask();
...
function sakaiRestoreNavigation(){
    ...
    $PBJQ('#toggleToolMenu').attr('title',$PBJQ('#toggleToolMax em').text());
    $PBJQ('#toggleNormal').css({
        'display': 'none'
    });
}
    ...
    if ($PBJQ('.' + id).length) {
        ...
        $PBJQ.getJSON(siteURL, function(data){
            $PBJQ.each(data, function(i, item){
                if (i <= maxToolsInt) {
                    var safeItemTitle = $PBJQ('<p></p>').text(item.title).html();
                    if (item.toolpopup) {
...
var setupToolToggle = function(toggleClass){
    ...
    $PBJQ('#toggleToolMenu').click(function(){
        ...
        $PBJQ('#subSites').filter(':visible').hide();
...
var setupSkipNav = function(){
    // function called from site.vm to enable skip links for all browsers
     $PBJQ('#skipNav a.internalSkip').click(function(){
         if ($PBJQ(this).attr('id') === 'openDrawer') {
            return dhtml_view_sites();
         }
         else {
