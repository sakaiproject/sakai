...
public class SectionCategoryProcessor extends AbstractCMProcessor {
    ...
    public void processRow(String[] data, ProcessorState state) throws Exception {
        log.debug("Reconciling section cateogory {}", data[0]);

        List<String> sectionCategories = cmService.getSectionCategories();

        if (!sectionCategories.contains(data[0])) {
            cmAdmin.addSectionCategory(data[0], data[1]);
        }
    }
