...
public class ADLSequencer implements SeqNavigation, SeqReportActivityStatus,
	...
	private void validateRequests() {
		...
		if (cur != null) {
			...
			if (cur.getControlModeChoiceExit() || !cur.getIsActive()) {
				...
				valid.mTreeModel = getTreeModel(mSeqTree.getRoot());
			}
	...
	private MutableTreeNode nodify(SeqActivity activity, boolean all) {
		MutableTreeNode node = new DefaultMutableTreeNode(activity);
		List<SeqActivity> children = activity.getChildren(all);
	 ...
	 * 
	 */
	public TreeModel getTreeModel(SeqActivity iStart) {
		...
		/*while (!done) {
			...
			MutableTreeNode parentNode = addNode(nodeMap, parent);
			
			if (null != cur && walk.getCount() == cur.getCount()) {
				curIdx = walk.getCount();
			}
						
			parentNode.setParent(addNode(nodeMap, walk));
			
			walk.setIsVisible(include);*/
...
//				parentTOC = temp.mParent;
				...
				while (walk == null && !done) {
					if (lookAt.size() > 0) {
						...
						temp = (ADLTOC) toc.elementAt(walk.getParentID());
		...
		if (toc != null && curIdx != -1) {
			...
			while (idx < toc.size()) {
				...
				if (temp.mParent == par) {
					if (!blocked) {
						SeqRuleset stopTrav = getActivity(temp.mID)
								.getPreSeqRules();
