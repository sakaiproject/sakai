...
public class FilePickerAction extends PagedResourceHelperAction
{
    ...
    private String buildAddMetadataContext(VelocityPortlet portlet, Context context, RunData data, SessionState state)
    {
		...
		else
		{
			...
			context.put("instruction", trb.getFormattedMessage("instr.create", typeDef.getLabel()));
			context.put("required", trb.getFormattedMessage("instr.require","<span class=\"reqStarInline\">*</span>"));
	...
	protected String buildSelectAttachmentContext(VelocityPortlet portlet, Context context, RunData data, SessionState state)
    {
		...
		if (!uploadCeiling.isBlank()) 
		{
			...
			{
				uploadCeilingNum = 20;
				log.debug("Can't parse upload ceiling property. Using default of 20 ");
			}
			try
			{
			...
			{
				uploadMaxNum = 20;
				log.debug("Can't parse upload max property. Using default of 20 ");
			}	
	...
	*/
	...
	public void doAttachupload(RunData data)
	{
		...
		else if (fileitem.getFileName().length() > 0)
		{
			...
			else if(fileContentStream != null)
			{
				...
				{
					addAlert(state, trb.getFormattedMessage("alert.virusfound", vfe.getMessage()));
				}
				catch (PermissionException e)
				{
				...
				catch(RuntimeException e)
				{
					if(ContentHostingService.ID_LENGTH_EXCEPTION.equals(e.getMessage()))
					{
						...
						addAlert(state, trb.getFormattedMessage("alert.toolong", name));
					}
					else
					{
	...
	public void doAttachurl(RunData data)
	{
		...
		{
			// invalid url
			addAlert(state, trb.getFormattedMessage("url.invalid", url) + crb.getString("validurl"));
		}
		catch (PermissionException e)
		{
			addAlert(state, crb.getString("notpermis4"));
		...
		catch(RuntimeException e)
		{
			...
			{
				// couldn't we just truncate the resource-id instead of rejecting the upload?
				addAlert(state, trb.getFormattedMessage("alert.toolong", url));
			}
			else
			{
	...
	*/
			...
			throws ToolException
	{
		...
		if (this.isGoogleDriveOn() && MODE_GOOGLEDRIVE.equals(toolSession.getAttribute(STATE_FILEPICKER_MODE)))
		{
			try {
				cleanup(state);
