		...
		if (contentType.equalsIgnoreCase(ResourceProperties.TYPE_URL))
		{
			try
			{
				...
				if (content != null && content.length > 0)
				{
					...
					{
						log.info(this + "getWebLinkResourceUrlString unsupportedencodingexception for " + new String(content) + " " + e.getMessage());
	
					}
					catch (URISyntaxException e)
					{
						log.info(this + "getWebLinkResourceUrlString error parsing URI for " + new String(content) + " " + e.getMessage());
					}
				}
			}
			catch (ServerOverloadException e)
			{
				log.info(this + "getWebLinkResourceUrlString cannot get content for resource ref=" + resource.getReference() + " error: " + e.getMessage());
			}
		}
