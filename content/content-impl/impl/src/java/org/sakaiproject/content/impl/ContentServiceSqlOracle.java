...
public class ContentServiceSqlOracle extends ContentServiceSqlDefault
{
	...
	{
		return "merge into CONTENT_DROPBOX_CHANGES using dual on (dual.dummy is not null and CONTENT_DROPBOX_CHANGES.DROPBOX_ID = ?) when not matched then insert values (DROPBOX_ID = ?, IN_COLLECTION = ?, LAST_UPDATE = ?) when matched then update set CONTENT_DROPBOX_CHANGES.IN_COLLECTION = ?, LAST_UPDATE = ?";
