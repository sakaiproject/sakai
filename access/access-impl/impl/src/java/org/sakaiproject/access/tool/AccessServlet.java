...
public class AccessServlet extends VmServlet
{
	...
	/** SAK-45537 Secure token parameter */
	...
	public void dispatch(HttpServletRequest req, HttpServletResponse res) throws ServletException
	{
		...
		try
		{
			...
			/** SAK-45537 Content: Ability to consume resources using time based tokens. */
			/*
			 * Time based tokens can be generated from different tools with the user's sessionId.
			 * If the current time is before the limit date of the file and the session is valid, we should provide access to the file no matter the permissions.
			 * Tokens have this format XXXXXXXXXXXXXXX|2021-05-25T15:38:00.955106300
			 * XXXXXXXXXXXXXXX is the session that must match with the current session
			 * 2021-05-25T15:38:00.955106300 is the validity date of the token, generated by the producer.
			 */
			...
			if (StringUtils.isNotBlank(secureTokenParameter)) {
				try {
					...
					log.debug("The secure token cannot be decrypted: {}", e.getMessage());
				}
			}
