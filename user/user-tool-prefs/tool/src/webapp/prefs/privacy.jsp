<%@ page import="java.util.*, jakarta.faces.context.*, jakarta.faces.application.*,
                 jakarta.faces.el.*, org.sakaiproject.user.tool.*"%>
...
<h:outputText value="#{Portal.latestJQuery}" escape="false"/>
	...
	<h:selectOneRadio value="#{privacyBean.defaultPrivacyStatus}" layout="pageDirection" styleClass="addRadioLabelPadding">
	...
	<h:panelGroup rendered="#{!privacyBean.myWorkspace || privacyBean.siteSelected}" >
      ...
      <f:verbatim></p></f:verbatim>
      <h:selectOneRadio value="#{privacyBean.privacyStatus}" layout="pageDirection" styleClass="addRadioLabelPadding">
	...
	--%>
	  <h:outputText escape="false" rendered="#{privacyBean.displayPopup}"
		value="<script>parent.privacy_show_popup();</script>" />
		  ...
		  <f:verbatim></h4></f:verbatim>
		<sakai:button_bar>
   			<h:commandButton action="#{privacyBean.processShowAll}" value="#{msgs.privacy_show_all}"
   				accesskey="s" title="#{msgs.privacy_show_title}" styleClass="active" />
   			<h:commandButton action="#{privacyBean.processHideAll}" value="#{msgs.privacy_hide_all}"
