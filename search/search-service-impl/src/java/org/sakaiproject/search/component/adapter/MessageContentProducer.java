...
import org.sakaiproject.search.component.dao.impl.SearchBuilderItemDaoImpl;
...
public class MessageContentProducer implements EntityContentProducer {
	 ...
	 * debug logger
	 */
	private static Log dlog = LogFactory.getLog(MessageContentProducer.class);
	...
	public String getContent(Entity cr) {
		...
		if (ep instanceof MessageService) {
			try {
				...
				dlog.info("Index Content is " + sb.toString());
				return sb.toString();
	...
	public Integer getAction(Event event) {
		...
		for (Iterator i = addEvents.iterator(); i.hasNext();) {
			...
			if (evt.equals(match)) {
				dlog.info(" event is add " + evt);
				return SearchBuilderItem.ACTION_ADD;
			} else {
				dlog.info(" no match " + evt + ":" + match);
			}
		}
		for (Iterator i = removeEvents.iterator(); i.hasNext();) {
			String match = (String) i.next();
			if (evt.equals(match)) {
				dlog.info(" event is delete " + evt);
				return SearchBuilderItem.ACTION_DELETE;
			} else {
				dlog.info(" no match " + evt + ":" + match);
			}
		}
		dlog.info(" event is unknown ");
		return SearchBuilderItem.ACTION_UNKNOWN;
	}
