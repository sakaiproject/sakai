...
/**
 * Represents an operation or stat of a document in the search engine.
 * This Object is used as a communication and persisntance mechanism 
 * between the changes made to entities and the thread processing the 
 * indec updates 
 * @author ieb
 *
 */
public interface SearchBuilderItem {
	...
	/**
	 * A master record is used to override the indexer threa operation and
	 * avoid hude updates to the database in the request cycle.
	 */
	...
	/**
	 * Action ADD the record to the search engine, if the doc ID
	 * is set, then remove first, if not set, check its not there.
	 */
	public static final Integer ACTION_ADD = new Integer(1);
	/**
	 * Action DELETE the record from the search engine, once complete
	 * delete the record
	 */
	public static final Integer ACTION_DELETE = new Integer(2);
	/**
	 * The action REBUILD causes the indexer thread to rebuild the
	 * index from scratch, refetching all entities
	 * This sould only ever appear on the master record
	 */
	public static final Integer ACTION_REBUILD = new Integer(11);
	/**
	 * The action REFRESH causes the indexer thread to refresh the
	 * search index from the current set of entities. If a Rebuild is
	 * in progress, the refresh will not overrise the rebuild
	 */
