 ...
 * using configurable {@link Fragmenter}, {@link Scorer}, {@link Formatter},
...
public class Highlighter
{
	...
	public Highlighter(Formatter formatter, Encoder encoder,
	 ...
	 * 
	 * @param analyzer
	 *        the analyzer that will be used to split <code>text</code> into
	 *        chunks
	 * @param text
	 ...
	 */
	public final String getBestFragment(Analyzer analyzer, String fieldName,
	 ...
	 * 
	 * @param tokenStream
	 *        a stream of tokens identified in the text parameter, including
	 *        offset information. This is typically produced by an analyzer
	 *        re-parsing a document's text. Some work may be done on retrieving
	 *        TokenStreams more efficently by adding support for storing
	 *        original text position data in the Lucene index but this support
	 *        is not currently available (as of Lucene 1.4 rc2).
	 * @param text
	 *        text to highlight terms in
	 * @return highlighted text fragment or null if no terms found
	 */
	 ...
	 *         of fragments)
	 */
	public final String[] getBestFragments(Analyzer analyzer, String text,
			int maxNumFragments) throws IOException
	{
			...
			String text, int maxNumFragments) throws IOException
	{
		...
		TextFragment[] frag = getBestTextFragments(tokenStream, text, true,
				maxNumFragments);
		...
		try
		{
			...
			while ((token = tokenStream.next()) != null)
			{
						...
						&& (tokenGroup.isDistinct(token)))
				{
					...
					String markedUpText = formatter.highlightTerm(encoder
							.encodeText(tokenText), tokenGroup);
					// store any whitespace etc from between this and last group
					if (startOffset > lastEndOffset)
						newText.append(encoder.encodeText(text.substring(
					...
					if (textFragmenter.isNewFragment(token))
					{
						...
						currentFrag = new TextFragment(newText, newText
								.length(), docFrags.size());
						fragmentScorer.startFragment(currentFrag);
						docFrags.add(currentFrag);
					}
				}
	...
	/**
	 * Highlights terms in the text , extracting the most relevant sections and
	 ...
	 */
	public final String getBestFragments(TokenStream tokenStream, String text,
	...
	/**
	 * @param byteCount
	 *        the maximum number of bytes to be tokenized per doc (This can
