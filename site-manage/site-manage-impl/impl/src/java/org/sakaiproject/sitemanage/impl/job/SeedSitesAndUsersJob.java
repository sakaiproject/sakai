		...
		for (Site site : sites.values()) {
			...
			for (int i = 0;i < numberOfGradebookItems;i++) {
				...
				try {
					ass.setName(name);
					ass.setPoints(100D);
					Long aid = gradebookService.addAssignment(siteId, siteId, ass);
					for (Member m : site.getMembers()) {
						gradebookService.saveGradeAndCommentForStudent(siteId, siteId, aid, m.getUserId(), Integer.toString(randomGenerator.nextInt(101)), faker.lorem().sentence(10));
					}
					if (i % 10 == 0) {
