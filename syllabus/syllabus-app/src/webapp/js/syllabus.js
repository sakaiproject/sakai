...
function showConfirmDeleteAttachment(deleteButton, msgs, event){
	...
	$('<div></div>').appendTo('body')
		.html('<div><div class="sak-banner-error">' + msgs.noUndoWarning + '</div><h6>' + msgs.confirmDelete + " '" + title + "'?</h6></div>")
		.dialog({
...
function showConfirmAdd(msgs, mainframeId){
	...
	$('<div></div>').appendTo('body')
		.html("<div><h6><span class='reqStar'>* </span>" + msgs.syllabus_title + "</h6><input type='text' id='newTitle'/></div><div style='display:none' id='requiredTitle' class='sak-banner-error'>" + msgs.required + "</div>" +
