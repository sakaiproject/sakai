...
public class VideoTrainingControllerHlsUploadTest {
    ...
    public void createVideo_createsPendingProcessJob_whenHlsUpload() throws Exception {
        ...
        when(uploadService.isManagedUploadProvider(VideoProviderType.HLS_UPLOAD)).thenReturn(true);
        ...
        String result = controller.createVideo("title", "desc", "", null, null, null, multipart, null, null, null, null, null, null, null, null, redirectAttributes, java.util.Locale.getDefault());

        verify(processJobService).queueManagedUploadJob(eq("v1"), eq("user1"), eq("/tmp/video-training/hls-1/upload-1.mp4"), eq(VideoProviderType.HLS_UPLOAD));
