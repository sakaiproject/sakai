...
import "/webcomponents/sakai-file-picker.js";
...
class SakaiGrader extends gradableDataMixin(SakaiElement) {
    ...
    return html`
        ...
        <div style="display: inline-block; margin-left: 20px;">
          ...
          <div id="grader-settings" @keydown=${this.onSettingsKeydown} class="settings">
            ...
            <div class="grader-groups">
              ...
              <select @change=${this.groupSelected}>
      ...
      <div class="grader-navigator">
        ...
        <div>
          ...
          ${this.anonymousGrading ? html`<span class="anonymous-grading-label">${this.i18n['anonymous_grading']}</span>` :
          html`<select @change=${this.studentSelected}>
    ...
    return html`
      ...
      <div class="grader ${this.graderOnLeft ? "on-left" : ""}">
        ...
        <div class="attempt-block">
          <span class="attempt-label">${this.i18n["attempt"]}:</span>
          <select>
            <option value="1">1</option>
            <option value="2">2</option>
          </select>
          <label>
          <input type="checkbox" @change=${this.allowResubmissionChanged}/>
        ...
        <div class="grade-block">
          ${this.gradeScale === "LETTER_GRADE_TYPE" ? html`
            ...
            <select @change=${this.gradeSelected}>
          ...
          ${this.gradeScale === "SCORE_GRADE_TYPE" ? html`
            <span>${this.i18n["grade"]}</span>
            <input id="grader-number-grade" @keydown=${this.validateGradeInput} @keyup=${this.gradeSelected} type="text" size="8" .value=${this.submission.grade} />
            ${this.renderSaved()}
          ...
          ${this.gradeScale === "CHECK_GRADE_TYPE" ? html`
            <input @click=${this.gradeSelected} type="checkbox" value="Checked" ?checked=${this.submission.grade === "Checked"}></input><span>${this.i18n["grade_checked"]}</span>
            ${this.renderSaved()}
          ` : html``}
          <!-- start hasAssociatedRubric -->
          ${this.hasAssociatedRubric === "true" ? html`
            <a href="javascript:;" id="grader-rubric-link" @click=${this.toggleRubric} title="${this.i18n["grading_rubric"]}"><span class="icon-sakai--sakai-rubrics"></span></a>
        ...
        <div class="feedback-label grader-label">
        ...
        <div class="feedback-attachments-block grader-label">
          ...
          <sakai-file-picker button-text="${this.i18n["add_attachments"]}" style="display: inline-block;" title="${this.i18n["add_attachments_tooltip"]}">
          </sakai-file-picker>
        </div>
        <div class="grader-label">
  ...
  allowResubmissionChanged(e) {
    this.allowResubmission = e.target.checked;
  }
  ...
  doneWithFeedbackDialog(e) {
    ...
    this.submission.feedbackComment = document.getElementById("grader-feedback-comment").value;
    this.toggleFeedback();
  ...
  doneWithPrivateNotesDialog(e) {
    ...
    this.privateNotes = document.getElementById("grader-private-notes").value
  ...
  getFormData() {
    ...
    this.querySelector("sakai-file-picker").files.forEach((f,i) => formData.set(`attachment${i}`, f, f.name));
    ...
    //formData.set("submissionId", this.submissionId);
    formData.set("submissionId", this.submission.id);
    formData.set("allowResubmission", this.allowResubmission ? "true" : "false");
    ...
    //console.log(this.submission.feedbackAttachments);
  ...
  submitGradingData(formData) {
    ...
    fetch("/direct/assignment/setGrade.json",
      ...
      .then(data => {
        ...
        this.querySelector("sakai-file-picker").reset();
  ...
  resetEditors(cancelling) {
    ...
    if (this.inlineFeedbackMode) {
      ...
      //this.feedbackTextEditor.setData(this.submission.feedbackText, function () { CKEDITOR.instances["grader-feedback-text-editor"].resetDirty(); });
    }
  }
  ...
  cancel() {
    ...
    //this.loadGradableData(this.gradableId, portal.siteId);
