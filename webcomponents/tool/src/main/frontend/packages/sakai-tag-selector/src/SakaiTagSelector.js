...
export class SakaiTagSelector extends SakaiShadowElement {
  ...
  _toggle(tag) {
    if (this._value.some(value => value.code === tag.code)) {
      this._value = this._value.filter(value => value.code !== tag.code);
    } else {
      const value = { name: tag.name, code: tag.code };
      if (tag.create) { this._options = [ ...this._options, value ]; }
  ...
  render() {
    ...
    return html`
      <div class="picker" @focusout=${event => {
      if (!event.relatedTarget || !this.shadowRoot.contains(event.relatedTarget)) { this._open = false; }
    }}>
        <div class="selector" aria-busy=${Boolean(this._loading)}>
          ${this._value.map(tag => html`
            <button class="tag" type="button" aria-label="${this._i18n.deselect}: ${tag.name}"
                ?disabled=${this._loading || this._error} @click=${() => this._toggle(tag)}>
          ...
          <input id="search" type="text" role="combobox" autocomplete="off"
              ...
              @input=${event => { this._query = event.target.value; this._active = -1; this._open = true; }}
        ...
        <ul id="options" role="listbox" aria-label=${label} aria-multiselectable="true" ?hidden=${!this._open}>
          ${choices.map((tag, index) => html`
            <li id="option-${index}" role="option" aria-selected=${this._value.some(value => value.code === tag.code)}
                ...
                @mousedown=${event => event.preventDefault()} @click=${() => this._toggle(tag)}>
        ...
        <span class="status" role="status">${this._value.length ? `${this._i18n.selected}: ${this._value.map(tag => tag.name).join(", ")}` : this._i18n.none_selected}</span>
      </div>
    `;
  }

  static styles = css`
    :host { display: block; min-width: 12rem; font: inherit; color: var(--sakai-text-color-1, #262626); }
    .picker { position: relative; }
    .selector { display: flex; flex-wrap: wrap; align-items: center; gap: .3rem; padding: .3rem; border: 1px solid var(--sakai-border-color, #767676); border-radius: .25rem; background: var(--sakai-background-color-1, white); }
    input { flex: 1; min-width: 10rem; padding: .3rem; border: 0; font: inherit; color: inherit; background: transparent; }
    button { font: inherit; cursor: pointer; }
    .tag { padding: .15rem .5rem; border: 1px solid var(--sakai-border-color, #767676); border-radius: 1rem; background: var(--infoBanner-bgcolor, #e6f2fa); color: var(--infoBanner-color, #174f78); }
    :focus-visible { outline: 2px solid var(--focus-outline-color, #005fcc); outline-offset: 2px; }
    ul { position: absolute; top: 100%; left: 0; right: 0; z-index: 1000; max-height: 14rem; overflow-y: auto; margin: 0; padding: 0; list-style: none; border: 1px solid var(--sakai-border-color, #767676); background: var(--sakai-background-color-1, white); }
    li { padding: .5rem; cursor: pointer; }
    li:hover, li.active { background: var(--sakai-background-color-2, #eee); }
    li[aria-selected="true"] { font-weight: bold; }
    p { margin: .3rem 0; }
