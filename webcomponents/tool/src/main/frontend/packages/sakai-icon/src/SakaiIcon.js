...
export class SakaiIcon extends LitElement {
  ...
  render() {
    return html`${icon(SakaiIcon.lookups.get(this.type), { classes: `sakai-${this.size}-icon` }).node}${this.hasAlerts ? html`<div class="alert"></div>` : nothing }`;
  }
...
SakaiIcon.lookups.set("microphone", faMicrophone);
...
SakaiIcon.lookups.set("video", faVideo);
...
SakaiIcon.lookups.set("search", faSearch);
