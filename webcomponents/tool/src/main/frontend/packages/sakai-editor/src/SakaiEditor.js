...
export class SakaiEditor extends SakaiElement {
  ...
  constructor() {
    ...
    this.toolbar = "full";
    this.content = "";
    this.loadTranslations("editor-wc").then(r => this._i18n = r);
  }
  ...
  render() {
    ...
    return html `
      <textarea style="width: 100%" id="${ifDefined(this.elementId)}" @input=${this._fireChanged} aria-label="Sakai editor textarea" tabindex="0">${this.content}</textarea>
    `;
  }
}
