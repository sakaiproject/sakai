...
describe("sakai-rubrics-list pagination", () => {
  it("should paginate rubrics correctly", async () => {
    ...
    // Should show first 10 rubrics on first page
    expect(el._paginatedRubrics.length).to.equal(10);
    expect(el._paginatedRubrics[0].title).to.equal("Rubric 1");
    expect(el._paginatedRubrics[9].title).to.equal("Rubric 10");
    expect(el._totalPages).to.equal(3);
    // Go to next page
    el._onPageSelected({ detail: { page: 2 } });
    await elementUpdated(el);
    expect(el._paginatedRubrics[0].title).to.equal("Rubric 11");
    expect(el._paginatedRubrics[9].title).to.equal("Rubric 20");
    // Go to last page
  ...
  it("should go to previous page if last item on last page is deleted", async () => {
    ...
    // 31 rubrics, 10 per page => 4 pages (10, 10, 10, 1)
    const rubrics = Array.from({ length: 31 }, (_, i) => ({
      ...data.rubric1,
      id: String(i + 1),
      title: `Rubric ${i + 1}`
    }));
    fetchMock.get(data.rubricsUrl, rubrics, { overwriteRoutes: true });
    const el = await fixture(html`<sakai-rubrics-list site-id="${data.siteId}"></sakai-rubrics-list>`);
    await waitUntil(() => el._rubrics && el._rubrics.length === 31, "Rubrics not loaded");
    await elementUpdated(el);
    // Go to last page
    el._onPageSelected({ detail: { page: 4 } });
    await elementUpdated(el);
    expect(el._paginatedRubrics.length).to.equal(1);
    // Simulate delete
    el._rubricDeleted({ detail: { id: "31" }, stopPropagation: () => {} });
    await elementUpdated(el);
    // Should go to previous page (page 3)
