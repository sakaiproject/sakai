...
describe("sakai-rubrics-shared-list pagination and actions", () => {
  ...
  it("should paginate shared rubrics correctly", async () => {
    ...
    // 13 rubrics, 5 per page => 3 pages (5, 5, 3)
    ...
    expect(el._paginatedRubrics[4].title).to.equal("Rubric 5");
    expect(el._totalPages).to.equal(3);
    // Go to last page
    el._onPageSelected({ detail: { page: 3 } });
    await elementUpdated(el);
    expect(el._paginatedRubrics.length).to.equal(3);
    expect(el._paginatedRubrics[0].title).to.equal("Rubric 11");
    expect(el._paginatedRubrics[2].title).to.equal("Rubric 13");
  });
  ...
  it("should go to previous page if last item on last page is deleted from shared", async () => {
    ...
    // 11 rubrics, 5 per page => 3 pages (5, 5, 1)
    const rubrics = Array.from({ length: 11 }, (_, i) => ({
      ...data.rubric1,
      id: String(i + 1),
      title: `Rubric ${i + 1}`
    }));
    ...
    const siteId = "site1";
    fetchMock.delete(`/api/sites/${siteId}/rubrics/11`, 200, { overwriteRoutes: true });
    const el = await fixture(html`<sakai-rubrics-shared-list site-id="${siteId}"></sakai-rubrics-shared-list>`);
    await waitUntil(() => el._rubrics && el._rubrics.length === 11, "Shared rubrics not loaded");
    await elementUpdated(el);
    // Go to last page
    el._onPageSelected({ detail: { page: 3 } });
    await elementUpdated(el);
    ...
    el.rubricIdToDelete = "11";
    await el.confirmDelete({ stopPropagation: () => {} });
    await elementUpdated(el);
    expect(el._currentPage).to.equal(2);
    expect(el._paginatedRubrics.length).to.equal(5);
    expect(el._paginatedRubrics[0].title).to.equal("Rubric 6");
  });
