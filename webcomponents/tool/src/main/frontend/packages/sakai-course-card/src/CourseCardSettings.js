...
export class CourseCardSettings extends SakaiDialog {
  ...
  _onImageSelected() { this._imageUpdated = true; }
  ...
  async _save() {
    ...
    if (this._imageMode) {
      ...
      if (this._imageUpdated) {
        ...
        blob = await imageEditor.getImageBlob(true);
  ...
  body() {
    ...
    return html`
      ...
      <div style="${this._imageMode ? "display: block;" : "display: none;"}">
        <sakai-image-editor image-url="${this.courseImage}" @image-selected=${this._onImageSelected} @image-updated=${this._onImageUpdated} hide-done-button></sakai-image-editor>
        <div class="d-flex align-center mt-1">
