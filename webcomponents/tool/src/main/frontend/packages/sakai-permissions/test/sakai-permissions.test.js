...
describe("sakai-permissions tests", () => {
  ...
  it ("renders correctly", async () => {
    ...
    expect(el.querySelectorAll("#permissions-container .row").length).to.equal(data.perms.available.length + 1);
    expect(el.querySelectorAll("#permissions-container .row:first-child div").length).to.equal(Object.keys(data.perms.on).length + 1);
