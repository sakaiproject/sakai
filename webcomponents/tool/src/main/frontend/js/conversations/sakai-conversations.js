...
export class SakaiConversations extends SakaiElement {
  ...
  static get properties() {
    ...
    return {
      ...
      _data: { attribute: false, type: Object },
      _addingTopic: { attribute: false, type: Boolean },
      _currentTopic: { attribute: false, type: Object },
      _topicBeingEdited: { attribute: false, type: Object },
      _showingSettings: { attribute: false, Boolean },
      _state: { attribute: false, type: String },
      _loadingData: { attribute: false, type: Boolean },
  ...
  _setStatePermissions() { this._state = STATE_PERMISSIONS; }

  _setStateManagingTags() { this._state = STATE_MANAGING_TAGS; }

  _setStateStatistics() { this._state = STATE_STATISTICS; }
  ...
  _renderSettingsMenu() {
    ...
    return html`
      <div class=${ifDefined(this._state === STATE_SETTINGS ? "setting-active" : undefined)}>
        <button class="btn btn-transparent" @click="${this._setStateSettings}">${this._i18n.general_settings}</button>
      </div>
      <div class=${ifDefined(this._state === STATE_PERMISSIONS ? "setting-active" : undefined)}>
        <button class="btn btn-transparent" @click="${this._setStatePermissions}">${this._i18n.permissions}</button>
      </div>
      ${this._data.canEditTags ? html`
      <div class=${ifDefined(this._state === STATE_MANAGING_TAGS ? "setting-active" : undefined)}>
        <button class="btn btn-transparent" @click="${this._setStateManagingTags}">${this._i18n.manage_tags}</button>
      </div>
      ` : ""}
      ${this._data.canViewSiteStatistics ? html`
      <div class=${ifDefined(this._state === STATE_STATISTICS ? "setting-active" : undefined)}>
        <button class="btn btn-transparent" @click="${this._setStateStatistics}">${this._i18n.statistics}</button>
      </div>
      ` : ""}
    `;
  }
  ...
  render() {
    ...
    return html`
      ...
      : html`
        ...
        <div id="conv-desktop">
          ...
          <div id="conv-topbar-and-content">
            ...
            <div id="conv-content">
              ${this._state === STATE_PERMISSIONS ? this._renderPermissions() : ""}
              ${this._state === STATE_MANAGING_TAGS ? this._renderTagManager() : ""}
              ${this._state === STATE_STATISTICS ? this._renderStatistics() : ""}
              ${this._state === STATE_SETTINGS ? this._renderGeneralSettings() : ""}
              ${this._state === STATE_ADDING_TOPIC ? this._renderAddTopic() : ""}
              ${this._state === STATE_DISPLAYING_TOPIC ? this._renderCurrentTopic() : ""}
      ...
      <div id="conv-mobile">
        ...
        ${this._state === STATE_NOTHING_SELECTED ? this._renderTopicList() : ""}
        ${this._state === STATE_DISPLAYING_TOPIC ? this._renderCurrentTopic() : ""}
        ${this._state === STATE_ADDING_TOPIC ? this._renderAddTopic() : ""}
        ${this._state === STATE_SETTINGS ? this._renderGeneralSettings() : ""}
        ${this._state === STATE_PERMISSIONS ? this._renderPermissions() : ""}
