...
public class AttemptDaoImpl extends HibernateDaoSupport implements AttemptDao {
	...
	public int count(final long contentPackageId, final String learnerId) {
		HibernateCallback hcb = new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				Criteria criteria = session.createCriteria(Attempt.class).add(Restrictions.eq("contentPackageId", contentPackageId))
	...
	public List<Attempt> find(long contentPackageId) {
		List r = getHibernateTemplate().find("from " + Attempt.class.getName() + " where contentPackageId=? ", new Object[] { contentPackageId });
	...
	public List<Attempt> find(long contentPackageId, String learnerId) {
		...
		List r = getHibernateTemplate().find(buffer.toString(), new Object[] { contentPackageId, learnerId });
	...
	public List<Attempt> find(String courseId, String learnerId) {
		...
		List r = getHibernateTemplate().find(buffer.toString(), new Object[] { courseId, learnerId });
	...
	public Attempt find(String courseId, String learnerId, long attemptNumber) {
		...
		List r = getHibernateTemplate().find(buffer.toString(), new Object[] { courseId, learnerId, attemptNumber });
