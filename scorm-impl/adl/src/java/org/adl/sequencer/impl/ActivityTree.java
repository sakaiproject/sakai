...
import org.adl.sequencer.IActivityTree;
 ...
 */
public class ActivityTree extends DefaultTreeModel implements Serializable, IActivityTree
{
    ...
    */
   private TreeNode mCurActivity = null;

   /**
   * This is the first candidate activity to for sequencing requests.
   */
   private TreeNode mFirstCandidate = null;
    ...
    * Defined in IMS SS AM
    */
   private TreeNode mSuspendAll = null;

   /**
    * This is an internal 'quick' reference map of the activity tree's nodes
    */
   private Map<String, TreeNode> activityMap = null;
    ...
    * Default constructor
    */
   public ActivityTree(TreeNode root)
   {
	   super(root);
   }
    ...
    */
   public ActivityTree(String iCourseID, 
                          String iLearnerID, 
                          String iScopeID,
                          SeqActivity iRoot)
   {
	  super(new DefaultMutableTreeNode(iRoot));
      if ( _Debug )
      {
   ...
   public void setLearnerID(String iLearnerID)
   {
      ...
      if ( !(activityMap == null || iLearnerID == null) )
      {

         Iterator it = activityMap.values().iterator();
...
*-/
   ...
   SeqActivity getActivityRoot()
   {
      ...
      return (SeqActivity)root.getUserObject();
   }
    ...
    */ 
   protected SeqActivity getCurrentActivity() {
	   return getSeqActivity(mCurActivity);
   }

   protected TreeNode getCurrentActivityNode() {
	   return mCurActivity;
   }
    ...
    */         
   protected SeqActivity getFirstCandidate() {
      ...
      return getSeqActivity(mFirstCandidate);
   }
   
   protected TreeNode getFirstCandidateNode() {
	   if (mFirstCandidate == null)
		   return mCurActivity;
    ...
    */
   public void setSuspendAll(TreeNode iSuspendTarget) {
    ...
    */
   public SeqActivity getActivity(String activityId) {
	   TreeNode activityNode = getActivityNode(activityId);
	   
	   return activityNode == null ? null : getSeqActivity(activityNode);
   }
   
   
   public TreeNode getActivityNode(String activityId) {   
	   ...
	   return activityId == null ? null : activityMap.get(activityId);
   }
   ...
   private void buildActivityMap()
   {
      ...
      {
         activityMap = new Hashtable<String, TreeNode>();
      }
      ...
      if ( root != null )
      {
    	  ...
    	  for (Enumeration<DefaultMutableTreeNode> e = node.breadthFirstEnumeration();e.hasMoreElements();) {
        	  DefaultMutableTreeNode currentNode = e.nextElement();
        	  
        	  SeqActivity activity = getSeqActivity(currentNode);
        	  
        	  // Add the current activity to the activity map
              activityMap.put(activity.getID(), currentNode);
          }
      }
   }
   ...
   private void scanObjectives() {
	   ...
	   for (Enumeration<DefaultMutableTreeNode> e = node.breadthFirstEnumeration();e.hasMoreElements();) {
		   ...
		   List objs = activity.getObjectives();
				...
				for (int i = 0; i < objs.size(); i++) {
					...
					if (obj.mMaps != null) {
						for (int j = 0; j < obj.mMaps.size(); j++) {
							SeqObjectiveMap map = (SeqObjectiveMap) obj.mMaps
									.get(j);
							...
							// If this is a 'read' objective add it to our obj
							// map
   ...
   public SeqActivity getSeqActivity(TreeNode node) {
	   SeqActivity activity = node == null ? null : (SeqActivity)((DefaultMutableTreeNode)node).getUserObject();
	   return activity;
   }
