...
During delivery, the SDS calls the validatePhase(phase, assessment, request) method on the selected module whenever the assessment enters a new phase. The delivery is allowed to continue if the module returns SUCCESS. If the module returns failure, the SDS sets the outcome to "secureDeliveryError" so that the secureDeliveryError.xhtml view is rendered. The decision to allow or disallow delivery to continue is entirely up to each module implementation. It can be something as simple as checking the browser agent to a full back-office integration that checks if the student has paid for the assessment.
...
START - deliverAssessment.xhtml, tableOfContents.xhtml
FINISH - submitted.xhtml
REVIEW - deliverAssessment.xhtml, histogramScores.xhtml
All phases - secureDeliveryError.xhtml

In addition to these views, the SDS injects one HTML fragment for each loaded module on the selectIndex_container.xhtml view by calling the getInitialHTMLFragment(request, locale) method on each module.
...
Plugin beans are loaded using the class loader used to load the samigo-app classes. If additional support jars are required, they can be placed in the tomcat/shared/lib directory.

==================================

Configure SAMigo to load the Assessment Blocker module:
Add samigo.secureDeliveryPlugins property to your sakai.properties file.
