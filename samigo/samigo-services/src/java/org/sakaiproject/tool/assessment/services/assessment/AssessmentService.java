...
public class AssessmentService {
	...
	private static final Predicate<Map.Entry<String, String>> ENTRY_IS_SAM_CORE = entry ->
			StringUtils.startsWith(entry.getKey(), CoreAssessmentEntityProvider.ENTITY_PREFIX + "/")
					&& StringUtils.startsWith(entry.getValue(), CoreAssessmentEntityProvider.ENTITY_PREFIX + "/");
...
//					}
	...
	private void linkGradebookCategory(String fromContext, String toContext, Map<String, String> transversalMap) {
		...
		try {
			gradingService.getCategoryDefinitions(fromContext).stream()
			...
			gradingService.getCategoryDefinitions(toContext).stream()
					.filter(category -> category.getId() != null && StringUtils.isNotBlank(category.getName()))
				...
				.forEach(entry -> {
					...
					try {
						...
						if (sourceAssessment != null && copiedAssessment != null) {
							...
							if (sourceEvaluation != null && StringUtils.equals(sourceEvaluation.getToGradeBook(), EvaluationModelIfc.TO_DEFAULT_GRADEBOOK.toString())) {
