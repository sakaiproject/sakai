...
public class SamigoExpressionParser {
	...
	public String parse(final String new_expr) {
		...
		try {
			...
			ans_str = new Double(ans).toString();
		}
		catch (Exception err) {
			ans_str = "Math expressin parser error occurred";
		}
  ...
  void getFirstChar() {
	 ...
	 */
	double eval_operator(final OPERATOR op_id, final double lhs, final double rhs) throws Error {
		switch (op_id) {
			// level 2
	 ...
	 * evaluate a function
	 */
	double eval_function(final String fn_name, final double value) throws Error {
		...
		if (fnUpper.equals("LN"))   {return Math.log(value);}
		if (fnUpper.equals("LOG")) {return Math.log10(value);}
	 ...
	 * evaluate a variable
	 */
	double eval_variable(final String var_name) throws Error {
