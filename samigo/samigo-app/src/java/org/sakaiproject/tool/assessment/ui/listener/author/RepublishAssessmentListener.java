	...
	private void updateGB(PublishedAssessmentFacade assessment) {
		...
		if (gbsHelper.gradebookExists(GradebookFacade.getGradebookUId(), g)) { 
			...
			if (evaluation.getToGradeBook() != null){
				if (StringUtils.equals(evaluation.getToGradeBook(), String.valueOf(EvaluationModelIfc.TO_DEFAULT_GRADEBOOK))
					...
					try {
						...
						try {
							...
							log.warn("Exception thrown while trying to create item in updateGB():" + e2.getMessage());
						}
					} catch (Exception e) {
