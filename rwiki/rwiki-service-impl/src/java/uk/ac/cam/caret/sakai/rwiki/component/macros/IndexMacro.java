...
public class IndexMacro extends BaseMacro {
    ...
    public void execute(Writer writer, MacroParameter params)
        ...
        }
        {
            ...
            while (it.hasNext()) {
                ...
                int nextSpaceIndex = nextName.lastIndexOf(NameHelper.SPACE_SEPARATOR);
                ...
                int index = 0;
                // lastSep should always be set to 0 at least (since spaces are normal)
                int lastSep = -1;
                ...
                if (index == shortSpaceLength && index == longerSpaceLength) {
                    ...
                    emitSpaceChange(writer, longerSpace, index, longerSpaceLength, isNextLonger);
                } else {
                    // We have a change of space which is both up and down
                    // Go down from the current space to the lastSep
                    emitSpaceChange(writer, currentNameChars, lastSep, currentSpaceIndex, false);
                    // Then go up to the next space
                    emitSpaceChange(writer, nextNameChars, lastSep, nextSpaceIndex, true);
                }
            ...
            if (currentSpaceIndex > space.length() ) {
                emitSpaceChange(writer, currentNameChars, space.length(), currentSpaceIndex, false);  
            }
            writer.write("  </ul>\n");
        }
    ...
    private void emitSpaceChange(Writer writer, char[] chars, int index, int end,
            boolean up) throws IOException {
