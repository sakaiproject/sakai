...
/**
 * The base class of all Entity references handled by the EntityBroker system.
 * This provides the minimal information of the entity prefix, which uniquely
 * identifies the {@link EntityProvider} responsible for handling the Entity.
 * 
...
public class EntityReference {
   ...
   public static final char SEPARATOR = '/';
	...
	public static int getSeparatorPos(String reference) {
		if (reference == null || reference.length() == 0 
				|| reference.charAt(0) != SEPARATOR) {
			throw new IllegalArgumentException(
					"Invalid entity reference for EntityBroker: " + reference
							+ " - these begin with /prefix, e.g. "
							+SEPARATOR+"myentity"+SEPARATOR+"3 OR "+SEPARATOR+"myentity");
		}
	 ...
	 * 
	 * @param reference a globally unique reference to an entity, consists of the entity prefix 
	 * and an optional suffix of unspecified size
   ...
   private static void checkPrefix(String prefix) {
      ...
      if (prefix.equals("")) {
         throw new IllegalArgumentException("prefix cannot be empty strings to get entity reference");
      }     
      if (prefix.indexOf(EntityReference.SEPARATOR) != -1) {
         throw new IllegalArgumentException("prefix cannot contain separator: " + EntityReference.SEPARATOR);
      }     
   }
