...
/** 
 * Represents a parsed form of a simple entity reference, as accepted to the
 * {@link EntityBroker} API. These are of the form /prefix/id - this parser will
 * accept overlong references with additional path segments, which will
 * be ignored.
 * 
...
public class IdEntityReference extends EntityReference {
	...
	/**
	 * A local entity id, represent an entity uniquely in a tool/webapp, could match with the actual id of a model data object 
	 */
	public String id;
	 ...
	 * 
	 * @param reference a globally unique reference to an entity, consists of the entity prefix and the local id
	 */
	 ...
	 * 
	 * @return the full entity reference or throw error if there is not enough information (prefix and id) to build it
	 */
	...
	public static String getID(String reference) {
		...
		if (spos == -1) {
         throw new IllegalArgumentException("cannot find id, separator position cannot be found, reference appears to have no id");		   
		}
	...
	private static void checkPrefixId(String prefix, String id) {
		...
		if (prefix.equals("") || id.equals("")) {
			throw new IllegalArgumentException("prefix and id cannot be empty strings to get entity reference");
		}		
      if (prefix.indexOf(EntityReference.SEPARATOR) != -1 ||
            id.indexOf(EntityReference.SEPARATOR) != -1) {
         throw new IllegalArgumentException("prefix and id cannot contain separator: " + EntityReference.SEPARATOR);
      }     
	}
