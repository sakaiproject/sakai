...
public class EntityBrokerDaoImplTest extends AbstractTransactionalSpringContextTests {
    ...
    public void testGetEntityRefsForSearch() {
        ...
        comparisons.clear();
        comparisons.add( ByPropsFinder.LIKE );
        results = dao.getEntityRefsForSearch(properties, values, comparisons, relations);
        assertNotNull(results);
        assertFalse(results.isEmpty());
        ...
        values.add( TestData.PROPERTY_NAME1 );
        comparisons.add( ByPropsFinder.EQUALS );
        relations.add("and");

        properties.add("propertyValue");
        values.add( TestData.PROPERTY_VALUE1 );
        comparisons.add( ByPropsFinder.EQUALS );
        relations.add("and");
    ...
    public void testDeleteProperties() {
        ...
        int removed = dao.deleteProperties(TestData.REF2, TestData.PROPERTY_NAME2);
        assertEquals(1, removed);
        
        // test removing all properties
        removed = dao.deleteProperties(TestData.REF1, null);
