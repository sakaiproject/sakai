...
/**
 * Common implementation of the handler for the EntityBroker system. This should
 * be used in preference to the EntityBroker directly by implementation classes
 * part of the EntityBroker scheme, rather than the user-facing EntityBroker
 * directly.
 * 
...
public class EntityHandler {
	 ...
	 * <br/>
	 * <b>Note:</b> the webapp name (relative URL path) of the direct servlet, of
	 * "/direct" is hardcoded into this method, and the {@link org.sakaiproject.entitybroker.servlet.DirectServlet} must
	 * be deployed there on this server. 
	 * 
	 * @param reference a globally unique reference to an entity, consists of 
	 * the entity prefix and the local id
	 * @return the full URL to a specific entity
	 */
	public String getEntityURL(String reference) {
        // try to get the prefix to ensure this is at least a valid formatted reference, should this make sure the entity exists?
	...
	public EntityProvider getProvider(String reference) {
		...
		EntityProvider provider = 
			entityProviderManager.getProviderByPrefix(prefix);
		return provider;
	}
	...
	public EntityReference parseReference(String reference) {
		...
		if (!(provider instanceof ReferenceParseable)) {
			...
			if (exemplar.getClass() == EntityReference.class) {
                ...
                throw new UnsupportedOperationException("Support for custom EntityReference classes is not yet supported");
            }
		}
	}
