...
/**
 * Implementation of the single HttpAccess instance handling all requests from
 * the AccessServlet to entities managed by the EntityBroker system. This class
 * is no longer used by default since EntityBroker no longer uses the
 * Sakai AccessServlet for exposing entities over Http.
 * 
...
public class EntityBrokerHttpAccess implements HttpAccess {
	...
	public void handleAccess(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			if (provider == null) {
				throw new IllegalArgumentException(
						"No access provider found for reference " + reference);
			}
		...
		try {
			...
			throw new EntityPermissionException(sessionManager
					.getCurrentSessionUserId(), "read", reference);
		}
