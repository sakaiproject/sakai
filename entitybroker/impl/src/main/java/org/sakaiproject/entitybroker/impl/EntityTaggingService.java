...
import org.sakaiproject.entitybroker.repository.EntityTagApplicationRepository;
...
public class EntityTaggingService implements TagSearchService {
   ...
   private EntityTagApplicationRepository entityTagApplicationRepository;
   public void setEntityTagApplicationRepository(EntityTagApplicationRepository entityTagApplicationRepository) {
      this.entityTagApplicationRepository = entityTagApplicationRepository;
   }
   ...
   public List<EntityData> findEntitesByTags(String[] tags, String[] prefixes,
      ...
      if (doSearch) {
         ...
         String[] prefixArray = validPrefixes.size() > 0 ? validPrefixes.toArray(new String[0]) : null;
         List<EntityTagApplication> tagApps = entityTagApplicationRepository.findByTagsAndPrefixes(tags, prefixArray, limit, start);
         if (matchAll) {
            // handle match all
      ...
      if (ref == null) {
         ...
         if (entityProviderManager.getProviderByPrefixAndCapability(ref.getPrefix(), Taggable.class) != null) {
            ...
            if (provider == null) {
               ...
               List<EntityTagApplication> results = entityTagApplicationRepository.findByEntityRef(reference);
         ...
         if (entityProviderManager.getProviderByPrefixAndCapability(ref.getPrefix(), Taggable.class) != null) {
            ...
            if (provider == null) {
               ...
               for (String tag : addTags) {
                  newTagApps.add( new EntityTagApplication(null, reference, ref.getPrefix(), tag) );
               }
               if (!newTagApps.isEmpty()) {
                  entityTagApplicationRepository.saveAll(newTagApps);
               }
            } else {
   ...
   public void removeTagsFromEntity(String reference, String[] tags) {
      ...
      if (ref == null) {
         ...
         if (entityProviderManager.getProviderByPrefixAndCapability(ref.getPrefix(), Taggable.class) != null) {
            ...
            if (provider == null) {
               // put in call to central tag system here if desired
               entityTagApplicationRepository.deleteByEntityRefAndTags(reference, tags);
            } else {
         ...
         if (entityProviderManager.getProviderByPrefixAndCapability(ref.getPrefix(), Taggable.class) != null) {
            ...
            if (provider == null) {
               ...
               if (!removeTags.isEmpty()) {
                  entityTagApplicationRepository.deleteByEntityRefAndTags(reference, removeTags.toArray(new String[0]));
               }
            } else {
