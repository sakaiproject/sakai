...
public class ReflectUtil {
    ...
    public static Map<Class<?>, Object> getPrimitiveDefaults() {
    ...
    private static void makePrimitiveDefaultsMap() {
       ...
       primitiveDefaults.put(char.class, (Character)'\u0000');
    ...
    public <T> T convert(Object object, Class<T> type) {
        ...
        if (type != null) {
            try {
                ...
                throw new UnsupportedOperationException("Could not convert object ("+object+") to type ("+type+"): " + e.getMessage(), e);
            }
        } else {
            if (type.isPrimitive()) {
                if (getPrimitiveDefaults().containsKey(type)) {
                    convert = (T) getPrimitiveDefaults().get(type);
                 }
            }
        }
        return convert;
