...
public abstract class AbstractEntityPropertiesService implements EntityPropertiesService {
    ...
    public void loadProperties(String prefix, String baseName, ClassLoader classLoader) {
        ...
        if (keys.size() > 0) {
            log.info("EntityPropertiesService: Added {} properties for entity prefix ({}) and basename ({})", keys.size(), prefix, baseName);
        } else {
            log.info("EntityPropertiesService: No properties to load for entity prefix ({}) and basename ({})", prefix, baseName);
        }
    }
