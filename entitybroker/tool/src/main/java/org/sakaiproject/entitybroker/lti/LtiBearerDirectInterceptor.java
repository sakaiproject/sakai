...
public class LtiBearerDirectInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (!sakaiAccessTokenService.isDirectApiEnabled()) {
            log.warn("LTI Bearer access to /direct is disabled ({}=false)",
                    SakaiAccessTokenService.PROPERTY_DIRECTAPI_ENABLED);
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "LTI Bearer access to /direct is disabled");
        }
