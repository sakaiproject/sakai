...
/**
 * Direct servlet allows unfettered access to entity URLs within Sakai, it also
 * handles authentication (login) if required (without breaking an entity URL)<br/>
 * This primarily differs from the access servlet in that it allows posts to
 * work and removes most of the proprietary checks
 * 
...
public class DirectServlet extends HttpServlet {
	...
	/**
	 * Checks dependencies and loads/inits them if needed<br/> <br/> Note: There
	 * is currently no way with the current component manager to check whether it
	 * is initialised without causing it to initialise. This method is here as a
	 * placeholder to invoke this function when it is available. All members which
	 * require the component manager to be initialised should be initialised in
	 * this method.
	 */
	private void checkDependencies() {
		try {
			...
			entityBroker = (EntityBroker) 
			  ComponentManager.get("org.sakaiproject.entitybroker.EntityBroker");
	...
	/**
	 * Handle the incoming request (get and post handled in the same way), passes
	 * control to the dispatch method or calls the login helper
	 * 
	 * @param req
	 *          (from the client)
	 * @param res
	...
	public void dispatch(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			if (! entityBroker.entityExists(path)) {
				log.warn("Attempted to access an entity URL path ("+path+") for an entity ("+ref.toString()+") that does not exist");
				...
				if (accessProvider == null) {
					log.warn("Attempted to access an entity URL path ("+path+") for an entity ("+ref.toString()+
							") when there is no HttpServletAccessProvider to handle the request for prefix ("+ref.prefix+")");
					sendError(res, HttpServletResponse.SC_NOT_FOUND);
				} else {
			...
			if (SessionManager.getCurrentSessionUserId() == null) {
				log.debug("Attempted to access an entity URL path ("+path+") for a resource which requires authentication without a session", e);
	 ...
	 *          (back to the client)
	 * @param path
	 *          current request path, set ONLY if we want this to be where to
