	...
	public void onInitialize() {
		...
		final AjaxButton backButton = new AjaxButton("backbutton") {
			...
			public void onSubmit(AjaxRequestTarget target) {
				...
				Component previousPanel = (assignmentsToCreate.size() > 0)
					? new CreateGradeItemStep(GradeImportConfirmationStep.this.panelId, Model.of(importWizardModel))
					: new GradeItemImportSelectionStep(GradeImportConfirmationStep.this.panelId, Model.of(importWizardModel));
			...
			public void onSubmit(AjaxRequestTarget target) {
				...
				itemsToSave.forEach(processedGradeItem -> {
					...
					for (ProcessedGradeItemDetail processedGradeItemDetail : processedGradeItemDetails) {
						...
						gradeDef.setGrade(FormatHelper.formatGradeForDisplay(processedGradeItemDetail.getGrade(), importWizardModel.getGradeType()));
