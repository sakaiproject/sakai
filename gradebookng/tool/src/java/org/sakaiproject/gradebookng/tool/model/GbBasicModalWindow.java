...
public class GbBasicModalWindow extends ModalWindow {
	...
	public GbBasicModalWindow(final String id) {
			super(id);
	...
	public void onInitialize() {
		...
		setWindowClosedCallback(new WindowClosedCallback() {
			...
			public void onClose(final AjaxRequestTarget target) {
				GbBasicModalWindow.this.closeCallbacks.forEach((callback) -> {
	...
	protected CharSequence getShowJavaScript() {
		final StringBuilder extraJavascript = new StringBuilder();

		// focus the first input field in the content pane
		extraJavascript.append(String.format(
				"setTimeout(function() {$('#%s :input:first:visible').focus();});",
				getContent().getMarkupId()));

		// position at the top of the page
		if (this.positionAtTop) {
			extraJavascript.append(String.format(
					"setTimeout(function() {sakai.gradebookng.spreadsheet.positionModalAtTop($('#%s').closest('.wicket-modal'));});",
					getContent().getMarkupId()));
		}

		return super.getShowJavaScript().toString()
				+ extraJavascript.toString();
	}
	...
	/**
	 * Set the component to return focus to upon closing the window. The
	 * component MUST have it's output markup id set, by calling
	...
	private void setDefaultWindowClosedCallback() {
		addWindowClosedCallback(new WindowClosedCallback() {
			...
			@Override
			public void onClose(final AjaxRequestTarget target) {
				// Disable all buttons with in the modal in case it takes a
				// moment to close
				...
				if (GbBasicModalWindow.this.componentToReturnFocusTo != null) {
					target.appendJavaScript(String.format(
							"setTimeout(function() {$('#%s').focus();});",
							GbBasicModalWindow.this.componentToReturnFocusTo
									.getMarkupId()));
				}
			}
		});
