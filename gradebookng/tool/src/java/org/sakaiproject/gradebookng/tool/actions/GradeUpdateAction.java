...
public class GradeUpdateAction extends InjectableAction implements Serializable {
	...
	public ActionResponse handleEvent(final JsonNode params, final AjaxRequestTarget target) {
		...
		GradeType gradeType = settings.getGradeType();
		if (StringUtils.isNotBlank(rawNewGrade)) {
			if (gradeType != GradeType.LETTER && (!NumberUtil.isValidLocaleDouble(rawNewGrade) || FormatHelper.validateDouble(rawNewGrade) < 0)) {
		...
		String oldGrade = gradeType == GradeType.LETTER ? rawOldGrade : FormatHelper.formatGradeFromUserLocale(rawOldGrade);
		final String newGrade = gradeType == GradeType.LETTER ? rawNewGrade : FormatHelper.formatGradeFromUserLocale(rawNewGrade);

		if (oldGrade.equals("")) oldGrade = null;
