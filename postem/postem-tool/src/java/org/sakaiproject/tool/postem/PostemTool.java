...
import org.sakaiproject.component.app.postem.data.Column;
...
*/
	...
	public String getCurrentStudentGrades() {
		this.userId = UsageSessionService.getSessionUserId();
		/*if (checkAccess()) {
			if (currentGradebook.getTemplate() != null) {
				return currentGradebook.getTemplate().getTemplateCode();
			} else {
				return "<p>No template currently exists for this gradebook.</p>";
			}
	...
	public String getFirstStudentGrades() {
		...
		if(students.size() == 0) {
			return "<p>No grades exist in gradebook "
				+ currentGradebook.getTitle() + ".</p>";
		}
	...
	public String processCreateNew() {
		try {
			...
			FacesContext.getCurrentInstance().addMessage(
					null,
					MessageUtils
							.getMessage(FacesMessage.SEVERITY_ERROR,
									"error_permission", (new Object[] { e
											.toString() }), FacesContext.getCurrentInstance()));
			return "permission_error";
		}
		this.userId = UsageSessionService.getSessionUserId();
		this.siteId = ToolManager.getCurrentPlacement().getContext();
		this.currentGradebook = gradebookManager.createEmptyGradebook(
				this.userId, this.siteId);
	...
	public String processGradebookUpdate() {
		...
		oldGradebook = gradebookManager.createEmptyGradebook(currentGradebook.getCreator(),
				currentGradebook.getContext());
	...
	public static void populateMessage(FacesMessage.Severity severity, String messageId, Object[] args) {
		...
		FacesContext.getCurrentInstance().addMessage(null,
				MessageUtils.getMessage(severity,
						messageId, args));
	}
	...
	public String processCreate() {
		...
		if (this.csv != null && this.csv.trim().length() > 0) {
			...
			try {
				...
				if (grades.getStudents() != null) {
					PostemTool.populateMessage(FacesMessage.SEVERITY_ERROR,
							"has_students", new Object[] { new Integer(grades
									.getStudents().size()) });
				}
				if (withHeader == true) {
				...
				/* TODO: properly subclass exception in order to allow for 
				 * localized messages (add getRowNumber/setRowNumber). Set exception message to be key in
				 * .properties file
				 */
				PostemTool.populateMessage(FacesMessage.SEVERITY_ERROR,
			...
			PostemTool.populateMessage(FacesMessage.SEVERITY_ERROR,
							"has_students", new Object[] { new Integer(0) });
		...
		if (this.newTemplate != null && this.newTemplate.trim().length() > 0) {
			currentGradebook.setTemplate(gradebookManager
		...
		/*if("No".equals(this.release)) {
			currentGradebook.setReleased(new Boolean(false));
			//logger.info("Set to No, " + currentGradebook.getReleased());
		} else {
			currentGradebook.setReleased(new Boolean(true));
			//logger.info("Set to Yes, " + currentGradebook.getReleased());
		}*/
	...
	public String getTitle() {
		return SiteService.findTool(PortalService.getCurrentToolId())
				.getTitle();
	}
