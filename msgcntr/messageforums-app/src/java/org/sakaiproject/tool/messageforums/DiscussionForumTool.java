    		...
    		if (message != null)
    		{
    			...
    			if(isRead || (decoTopic.getIsNewResponse() && decoMsg.getIsOwn()))
    			{
    				...
    				if (readStatus != null) {
    					...
    					decoMsg.setRead(messageManager.isMessageNotReadForUser(topic.getId(),
    							message.getId()));
    				}
    				boolean isOwn=false;
    				if(getUserId()!=null){
  ...
  { 
   ...
   private String gotoMain() {
	...
	private boolean isForumsPageInSite(String siteId) {
	 ...
	 {
		 return messageManager.isMessageNotReadForUser(topicId, messageId);
	 }
	 ...
	 public String processActionDisplayInThread() {
		 ...
		 if (uiPermissionsManager.isRead((DiscussionTopic)topic, forum)) {
			 ...
			 while(messageIter.hasNext()){
				 ...
				 messageManager.markMessageNotReadForUser(topic.getId(), mes.getId(), true, getUserId());
			 }
		 }
