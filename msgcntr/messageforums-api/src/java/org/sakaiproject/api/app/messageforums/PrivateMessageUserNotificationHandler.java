        ...
        if (openTime == null || openTime.before(new Date()) && !pvtMessage.getDraft()) {
            try {
                ...
                if (!from.equals(to)) {
                    ...
                    notificationEvents.add(new UserNotificationData(from, to, siteId, title, url, DiscussionForumService.MESSAGES_TOOL_ID));
                }
    ...
    private List<UserNotificationData> handleAdd(String from, String siteId, PrivateMessage pvtMessage) {
        ...
        try {
            ...
            return pvtMessage.getRecipients()
                ...
                .map(r -> new UserNotificationData(from, r.getUserId(), siteId, title, url, DiscussionForumService.MESSAGES_TOOL_ID))
