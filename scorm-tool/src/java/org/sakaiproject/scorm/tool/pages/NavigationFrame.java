...
public class NavigationFrame extends WebPage {	
	...
	public NavigationFrame(PageParameters pageParams) {
		...
		final ISequencer sequencer = null; //clientFacade.getSequencer(manifest);
			...
			protected Component newNodeComponent(String id, IModel model)
			{
				return new LinkIconPanel(id, model, this)
				{
					...
					protected void onNodeLinkClicked(TreeNode node, BaseTree tree, AjaxRequestTarget target)
					{
						...
						if (null != launchData) {
							String url = manifest.getResourceId();
					...
					{
						ISeqActivity item = (ISeqActivity)((DefaultMutableTreeNode)model.getObject()).getUserObject();
						
						return new Label(componentId, item.getTitle());
					}
				};
			}
