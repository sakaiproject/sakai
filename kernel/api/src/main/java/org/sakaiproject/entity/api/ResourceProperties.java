...
import org.sakaiproject.entity.api.exception.EntityPropertyNotDefinedException;
import org.sakaiproject.entity.api.exception.EntityPropertyTypeException;
...
public interface ResourceProperties extends Serializable {
	public static enum PropertyName {
		/**
		 * Property for resource creator (uploader) (automatic). [user id
		 * string]
		 */
		PROP_CREATOR("CHEF:creator"),

		/**
		 * Property for resource last one to modify (automatic). [user id
		 * string]
		 */
		PROP_MODIFIED_BY("CHEF:modifiedby"),

		/** Property for creation (upload) date (live, from DAV:). [Time] */
		PROP_CREATION_DATE("DAV:creationdate"),

		/**
		 * Property for the display name (description) (dead, from DAV:).
		 * [String]
		 */
		PROP_DISPLAY_NAME("DAV:displayname"),

		/** Property for the original filename (automatic). [String] */
		PROP_ORIGINAL_FILENAME("CHEF:originalfilename"),

		/** Property for the copyright attribution (user settable). [String] */
		PROP_COPYRIGHT("CHEF:copyright"),

		/**
		 * Property for the copyright choice attribution (user settable).
		 * [String]
		 */
		PROP_COPYRIGHT_CHOICE("CHEF:copyrightchoice"),

		/**
		 * Property for the copyright alert attribution (user settable).
		 * [String]
		 */
		PROP_COPYRIGHT_ALERT("CHEF:copyrightalert"),

		/** Property for the content length (live, from DAV:). [Long] */
		PROP_CONTENT_LENGTH("DAV:getcontentlength"),

		/** Property for the content type (live, from DAV:). [MIME type string] */
		PROP_CONTENT_TYPE("DAV:getcontenttype"),

		/**
		 * Property for the last modified date (live, from DAV:, set when
		 * anything changes). [Time]
		 */
		PROP_MODIFIED_DATE("DAV:getlastmodified"),

		/**
		 * Property that distinguishes a collection from a non-collection
		 * resource (automatic). [Boolean]
		 */
		PROP_IS_COLLECTION("CHEF:is-collection"),

		/**
		 * Property that holds a ContentHosting collection body bytes quota, in
		 * K (user settable). [long]
		 */
		PROP_COLLECTION_BODY_QUOTA("CHEF:collection-body-quota"),

		/**
		 * Property to associate a chat message with a chat room (user
		 * settable). [String]
		 */
		PROP_CHAT_ROOM("CHEF:chat-room"),
		...
		PROP_DESCRIPTION("CHEF:description"),

		/** Property for calendar event types (user settable). [String] */
		PROP_CALENDAR_TYPE("CHEF:calendar-type"),

		/** Property for calendar event location (user settable). [String] */
		PROP_CALENDAR_LOCATION("CHEF:calendar-location"),

		/**
		 * Property for the channel to categories names inside a discussion
		 * channel (user settable). [String]
		 */
		PROP_DISCUSSION_CATEGORIES("CHEF:discussion-categories"),

		/**
		 * Property that discussion reply message style is star or thread
		 * (automatic) [String]
		 */
		PROP_REPLY_STYLE("CHEF:discussion-reply-style"),

		/**
		 * Property for a message channel indicating if the channel is 'enabled'
		 * (user settable) [Boolean]
		 */
		PROP_CHANNEL_ENABLED("CHEF:channel-enabled"),

		/**
		 * Property for a message channel indicating if the channel is 'enabled'
		 * (user settable) [Boolean]
		 */
		PROP_MAIL_CHANNEL_OPEN("CHEF:mail-channel-open"),

		/**
		 * Property for a site storing the email notification id associated with
		 * the site's mailbox (user settable) [String]
		 */
		PROP_SITE_EMAIL_NOTIFICATION_ID("CHEF:site-email-notification-id"),

		/**
		 * Property for a site indicating if email archiveing is enabled for the
		 * site (user settable) [Boolean]
		 */
		PROP_SITE_EMAIL_ARCHIVE("CHEF:site-email-archive"),

		/**
		 * Property for a ToolRegistration, the title of the tool (user
		 * settable) [String]
		 */
		PROP_TOOL_TITLE("CHEF:tool-title"),

		/**
		 * Property for a ToolRegistration, description of the tool (user
		 * settable) [String]
		 */
		PROP_TOOL_DESCRIPTION("CHEF:tool-description"),

		/**
		 * Property for a ToolRegistration, category of the tool (user settable)
		 * [String] %%% list desired -ggolden
		 */
		PROP_TOOL_CATEGORY("CHEF:tool-category"),

		/** Property for calendar event extra fields (user settable). [String] */
		PROP_CALENDAR_EVENT_FIELDS("CHEF:calendar-fields"),

		/**
		 * Property for whether an assignment's open date will be announced
		 * (user settable). [String]
		 */
		NEW_ASSIGNMENT_CHECK_AUTO_ANNOUNCE("new_assignment_check_auto_announce"),

		/**
		 * Property for whether an assignment's due date will be added into
		 * schedule as an event(user settable). [String]
		 */
		NEW_ASSIGNMENT_CHECK_ADD_DUE_DATE("new_assignment_check_add_due_date"),
		...
		PROP_ASSIGNMENT_DUEDATE_CALENDAR_EVENT_ID(
				"CHEF:assignment_duedate_calender_event_id"),

		/**
		 * Property for announcement message id associated with an assignment's
		 * open date (user settable). [String]
		 */
		 ...
		 * [String]
		 */
		PROP_SUBMISSION_PREVIOUS_GRADES("CHEF:submission_previous_grades"),

		/**
		 * Property for assignment submission's scaled previous grade (user
		 * settable). [String]
		 */
		PROP_SUBMISSION_SCALED_PREVIOUS_GRADES(
				"CHEF:submission_scaled_previous_grades"),
		...
		PROP_SUBMISSION_PREVIOUS_FEEDBACK_TEXT(
				"CHEF:submission_previous_feedback_text"),

		/**
		 * Property for assignment submission's previous feedback comment (user
		 * settable). [String]
		 */
		PROP_SUBMISSION_PREVIOUS_FEEDBACK_COMMENT(
				"CHEF:submission_previous_feedback_comment"),

		/** Property for assignment been deleted status(user settable) [String] */
		PROP_ASSIGNMENT_DELETED("CHEF:assignment_deleted"),

		/** Property indicating viewable (manual). [Boolean] */
		PROP_PUBVIEW("SAKAI:pubview"),

		/** The encoding of the resource - UTF-8 or ISO-8559-1 for example */
		PROP_CONTENT_ENCODING("encoding"),

		/** Property for "object type" of a structured artifact */
		PROP_STRUCTOBJ_TYPE("SAKAI:structobj_type"),

		/**
		 * Used to find non structured object ContentResources (files, url's,
		 * etc.)
		 */
		FILE_TYPE("fileResource"),

		/**
		 * Property name on a ContentEntity indicating its rank if a custom
		 * priority sort is used.
		 */
		PROP_CONTENT_PRIORITY("SAKAI:content_priority"),

		/**
		 * Property name on a ContentCollection indicating that a custom
		 * priority sort is to be used.
		 */
		PROP_HAS_CUSTOM_SORT("SAKAI:has_custom_sort"),

		/**
		 * Property name on a ContentEntity indicating the ResourceType that
		 * defines its properties.
		 */
		PROP_RESOURCE_TYPE("SAKAI:resource_type");
	 ...
	 * 
	 * @return An iterator on the names of the defined properties (Strings) (may
	 *         be empty).
	 */
	...
	/**
	 * Access a named property as a List of (String), good for single or
	...
	/**
	 * Access a named property; as a String if it's single valued, or a List of
	 * (String) if it's multi-valued (or null if it's not defined).
	 * 
	 * @param name
	 *            The property name.
	 * @return the property value, or null if not found.
	 ...
	 */
	long getLongProperty(String name) throws EntityPropertyNotDefinedException,
			EntityPropertyTypeException;
	 ...
	 */
	Time getTimeProperty(String name) throws EntityPropertyNotDefinedException,
			EntityPropertyTypeException;
	 ...
	 */
	// TODO: -ggolden User getUserProperty(String name) throws
