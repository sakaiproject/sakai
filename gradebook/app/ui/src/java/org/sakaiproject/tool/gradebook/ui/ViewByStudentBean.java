...
public class ViewByStudentBean extends EnrollmentTableBean implements Serializable {
    ...
    static {
        ...
        pointsEarnedComparator = new Comparator() {
            @Override
			public int compare(final Object o1, final Object o2) {
            	final int comp = doubleOrNothingComparator.compare(((AssignmentGradeRow)o1).getPointsEarned(), ((AssignmentGradeRow)o2).getPointsEarned());
                if (comp == 0) {
        ...
        gradeAsPercentageComparator = new Comparator() {
            @Override
			public int compare(final Object o1, final Object o2) {
            	final int comp = doubleOrNothingComparator.compare(((AssignmentGradeRow)o1).getGradeAsPercentage(), ((AssignmentGradeRow)o2).getGradeAsPercentage());
                if (comp == 0) {
					return nameComparator.compare(o1, o2);
				} else {
					return comp;
        ...
        itemValueComparator = new Comparator() {
            @Override
			public int compare(final Object o1, final Object o2) {
            	final int comp = doubleOrNothingComparator.compare(((AssignmentGradeRow)o1).getAssociatedAssignment().getPointsPossible(), ((AssignmentGradeRow)o2).getAssociatedAssignment().getPointsPossible());
                if (comp == 0) {
					return nameComparator.compare(o1, o2);
				} else {
					return comp;
	...
	public void init() {
    	...
    	final List<AssignmentGradeRecord> studentGradeRecs = getGradebookManager().getStudentGradeRecords(this.gradebook.getId(), this.studentUid);
    	...
    	this.totalPoints = getGradebookManager().getTotalPointsInternal(this.gradebook, getGradebookManager().getCategories(this.gradebook.getId()),
     ...
     * @return
     */
    private List retrieveGradeRows(List assignments, final List gradeRecords) {
		...
		if (userHasGraderPerms) {
			viewableAssignmentsMap = getGradebookPermissionService().getAvailableItemsForStudent(this.gradebook.getId(), getUserUid(), this.studentUid, getAllSections());
		}
    	...
    	final Map goEventListMap = getGradebookManager().getGradingEventsForStudent(this.studentUid, assignments);
    	...
    	final Iterator assignmentIterator = assignments.iterator();
		...
		if (this.isInstructorView) {
			final Map<String, List<String>> visible = gext.getVisibleExternalAssignments(this.gradebook.getUid(), Arrays.asList(this.studentUid));
    ...
    private void initializeStudentGradeData() {
    	...
    	if (this.assignmentsReleased || this.isInstructorView) {
    		...
    		if (getCategoriesEnabled()) {
    			...
    			if (this.sortColumn.equals(Category.SORT_BY_WEIGHT)) {
					categoryListWithCG = getGradebookManager().getCategoriesWithStats(getGradebookId(), GradebookAssignment.DEFAULT_SORT, true, this.sortColumn, this.sortAscending, false, studentUids);
				} else {
				...
				//SAK-19896, eduservice's can't share the same "Category" class, so just pass the ID's
    				...
    				final List<Long> viewableCats = getGradebookPermissionService().getCategoriesForUserForStudentView(getGradebookId(), getUserUid(), this.studentUid, catIds, getViewableSectionIds());
    			...
    			if (!isUserAbleToGradeAll() && (isUserHasGraderPermissions() && !getGradebookPermissionService().getPermissionForUserForAllAssignmentForStudent(getGradebookId(), getUserUid(), this.studentUid, getViewableSectionIds()))) {
    				// user is not authorized to view/grade the unassigned category for the current student
    			} else {
    				final List assignNoCat = getGradebookManager().getAssignmentsWithNoCategory(getGradebookId(), GradebookAssignment.DEFAULT_SORT, true);
