...
public class SpreadsheetUploadBean extends GradebookDependentBean implements Serializable {
    ...
    public String getVerifyColumnCount() {
    	final int intColumnCount = new Integer(this.columnCount == null ? "0" : this.columnCount).intValue() - 1;
    ...
    public String processFileEntire() throws Exception {
        ...
        if (this.upFile != null) {
	        if (this.upFile != null && log.isDebugEnabled()) {
	            log.debug("file size " + this.upFile.getSize() + "file name " + this.upFile.getName() + "file Content Type " + this.upFile.getContentType() + "");
	        }
        ...
        while (assignIter.hasNext()) {
        	...
        	if (assignment == null) {
        		...
        		if (pointsPossible != null) {
        			...
        			this.assignmentId = getGradebookManager().createAssignment(getGradebookId(), assignmentName, pointsPossible, null, Boolean.FALSE, Boolean.TRUE, Boolean.FALSE);
               			...
               			if (inputScore != null && inputScore.trim().length() > 0) {
                            ...
                            if (getGradeEntryByPoints() || getGradeEntryByPercent()) {
                                try {
                                    ...
                                    if(log.isDebugEnabled()) {
										log.debug("user "+user + " userid " + userid +" score "+inputScore.toString());
									}
     ...
     * 			have changed
     */
    private List gradeChanges(final GradebookAssignment assignment, final List fromSpreadsheet, final int index, final LetterGradePercentMapping lgpm) {
    ...
    public String saveGrades(){
        ...
        try {
        	...
        	if (newCategory != null) {
        		this.assignmentId = getGradebookManager().createAssignmentForCategory(getGradebookId(), newCategory.getId(), this.assignment.getName(), this.assignment.getPointsPossible(), this.assignment.getDueDate(), new Boolean(this.assignment.isNotCounted()),new Boolean(this.assignment.isReleased()), new Boolean(this.assignment.isExtraCredit()));
        	} else {
        		this.assignmentId = getGradebookManager().createAssignment(getGradebookId(), this.assignment.getName(), this.assignment.getPointsPossible(), this.assignment.getDueDate(), new Boolean(this.assignment.isNotCounted()),new Boolean(this.assignment.isReleased()), new Boolean(this.assignment.isExtraCredit()));
        	}
    ...
    public class Spreadsheet  implements Serializable {
        ...
        public void setGradebookId(final Long gradebookId) {
    ...
    public class SpreadsheetHeader implements Serializable{
		...
		public void setHasCumulative(final boolean hasCumulative) {
    ...
    public class SpreadsheetRow implements Serializable {
        ...
        public void setRowcontent(final List rowcontent) {
	...
	private int getNumStudentsInSite() {
        ...
        final int numStudentsInSite = enrollments != null ? enrollments.size() : 0;
