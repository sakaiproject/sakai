...
public class SakaiLTIUtil {
	...
	private static Object handleGradebook(String sourcedid, HttpServletRequest request,
		...
		try {
			...
			if (isRead) {
				...
				gradingService.setAssignmentScoreString(gradebookColumn.getGradebookUid(), siteId, gradebookColumn.getId(), user_id, null, "External Outcome", null);
				...
				gradingService.setAssignmentScoreString(gradebookColumn.getGradebookUid(), siteId, gradebookColumn.getId(), user_id, gradeI18n, "External Outcome", null);
	...
	public static Object handleGradebookLTI13(Site site,  Long tool_id, Map<String, Object> content, String userId,
		...
		try {
			...
			sess.setUserEid(gb_user_eid);
			if (scoreGiven == null) {
				gradingService.setAssignmentScoreString(gradebookColumn.getGradebookUid(), siteId, gradebookColumn.getId(), userId, null, "External Outcome", null);
				...
				gradeI18n = (",").equals((ComponentManager.get(FormattedText.class)).getDecimalSeparator()) ? gradeI18n.replace(".",",") : gradeI18n;
				gradingService.setAssignmentScoreString(gradebookColumn.getGradebookUid(), siteId, gradebookColumn.getId(), userId, gradeI18n, "External Outcome", null);
				if ( StringUtils.isBlank(comment) ) {
					gradingService.deleteAssignmentScoreComment(gradebookColumn.getGradebookUid(), gradebookColumn.getId(), userId);
				} else {
					gradingService.setAssignmentScoreComment(gradebookColumn.getGradebookUid(), gradebookColumn.getId(), userId, comment);
