...
public interface LtiToolRepository extends SpringCrudRepository<LtiTool, Long> {
     ...
     * Admins see every tool. Otherwise the result contains tools owned by the requesting site, globally
     * visible system tools (including stealthed ones when {`@code` includeStealthed} is true), and, when
     * launchable tools are requested, tools deployed to the site via lti_tool_site.
     *
     * `@param` siteId the requesting site (may be null for admins)
     * `@param` isAdmin whether the request is made with an admin role
