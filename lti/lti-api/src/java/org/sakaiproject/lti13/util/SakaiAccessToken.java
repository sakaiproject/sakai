...
public class SakaiAccessToken extends BaseJWT {
	...
	/** OAuth/SAT scope for an LTI API permission (e.g. {@code content.read} → {@code sakai.lti.api.content.read}). */
	public static String permissionToLtiApiScope(String permission) {
		if (StringUtils.isBlank(permission)) {
			log.warn("permission cannot be null or empty. Returning null token scope ...");
			return null;
		}
		return SCOPE_LTI_API_PREFIX + permission;
	}

	/** Permission from an LTI API scope, or null if {@code scope} is not under {@link #SCOPE_LTI_API_PREFIX}. */
	public static String ltiApiScopeToPermission(String scope) {
		if (scope == null || !scope.startsWith(SCOPE_LTI_API_PREFIX)) {
			return null;
		}
		String permission = scope.substring(SCOPE_LTI_API_PREFIX.length());
		return permission.length() > 0 ? permission : null;
	}
}
