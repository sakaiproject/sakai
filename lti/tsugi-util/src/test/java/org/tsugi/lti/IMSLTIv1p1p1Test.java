 ...
 * the IMS LTI 1.1 Basic Outcomes specification examples verbatim.
 * 
 ...
 * 
 * NOTE: The v1.1.1 Basic Outcomes spec examples include empty <imsx_codeMinor/>
 * elements in response XML (see Figures 4, 6, 8). This differs from the v1.1
 * Implementation Guide examples (see IMSLTIv1p1Test.java), which do NOT include
 * empty codeMinor elements. Our code correctly omits null codeMinor fields in both
 * cases via @JsonInclude(NON_NULL), so normalization removes empty codeMinor from
 * the v1.1.1 spec examples to align with our correct implementation.
 */
public class IMSLTIv1p1p1Test {
    ...
    // ========================================================================
    // Canonical XML examples from the LTI v1.1.1 Basic Outcomes spec
    ...
    /** XML from Figure 4 / Section 6.1.1 - replaceResultResponse */
    ...
    /** XML from Figure 6 / Section 6.1.2 - readResultResponse */
    ...
    /** XML from Figure 8 / Section 6.1.3 - deleteResultResponse */
     ...
     * except for the documented codeMinor normalization below.
     * 
     ...
     * 
     * SPECIAL CASE - codeMinor normalization:
     * The v1.1.1 Basic Outcomes spec examples show empty <imsx_codeMinor/> elements
     * in response XML (see Figures 4, 6, 8). This differs from the v1.1 Implementation
     * Guide examples (see IMSLTIv1p1Test.java), which do NOT include empty codeMinor elements.
     * Our code correctly omits null codeMinor fields (via @JsonInclude(NON_NULL) on
     * POXStatusInfo.codeMinor). An empty codeMinor semantically means "no minor code",
     * same as omitting it. This normalization aligns the v1.1.1 spec examples with our
    ...
    private String normalizeToJacksonStyle(String xml) {
        ...
        // Step 2: Normalize empty elements to self-closing form
        ...
        // Step 3 & 4: Normalize whitespace using utility
        ...
        // Step 5: Remove empty codeMinor elements
        // JUSTIFICATION: The v1.1.1 Basic Outcomes spec examples show <imsx_codeMinor/> (empty element
        // meaning "no minor code"). This differs from the v1.1 Implementation Guide examples
        // (see IMSLTIv1p1Test.java), which do NOT include empty codeMinor elements.
        // Our objects correctly omits null codeMinor fields via @JsonInclude(NON_NULL).
        // Semantically equivalent: empty codeMinor = omitted codeMinor = "no minor code".
        // This normalization aligns the v1.1.1 spec examples with our correct implementation.
        // NOTE: This is the ONLY element removal in normalization - all others are syntax-only.

        // From the IMS Basic Outcomes Schema:
        // The "imsx_statusInfo" element (used in readResultResponse/replaceResultResponse etc.)
        // is defined in the IMS General Web Services (GWS) schema, which is imported by the
        // LTI 1.1 Basic Outcomes schema.
        //
        // The "imsx_codeMinor" element is optional in the GWS statusInfo type, meaning
        // it may be present 0..1 times according to the XSD (i.e., its maxOccurs="1" and
     ...
     * 
     * NOTE: Empty null fields are already correctly omitted by our Java classes
     * via @JsonInclude(NON_NULL) annotations, so no element removal is needed here.
     */
    ...
    /**
     * Test replaceResultRequest - matches Figure 3 from 
     * https://www.imsglobal.org/spec/lti-bo/v1p1
     * and Section 6.1.1 from https://www.imsglobal.org/specs/ltiv1p1p1/implementation-guide
     */
    @Test
    public void testReplaceResultRequest() throws Exception {
        // XML from Figure 3 of the spec
        ...
        assertEquals("replaceResultRequest should match Figure 3", 
                normalizedSpec, normalizedGenerated);
    }
    
    /**
     * Test replaceResultResponse - matches Figure 4 from 
     * https://www.imsglobal.org/spec/lti-bo/v1p1
     * and Section 6.1.1 from https://www.imsglobal.org/specs/ltiv1p1p1/implementation-guide
     */
    @Test
    public void testReplaceResultResponse() throws Exception {
        // XML from Figure 4 of the spec
        ...
        assertEquals("replaceResultResponse should match Figure 4", 
                normalizedSpec, normalizedGenerated);
    }
    
    /**
     * Test readResultRequest - matches Figure 5 from 
     * https://www.imsglobal.org/spec/lti-bo/v1p1
     * and Section 6.1.2 from https://www.imsglobal.org/specs/ltiv1p1p1/implementation-guide
     */
    @Test
    public void testReadResultRequest() throws Exception {
        // XML from Figure 5 of the spec
        ...
        String normalizedSpec = normalizeToJacksonStyle(specXml);
        String normalizedGenerated = normalizeGeneratedXml(generatedXml);
        assertEquals("readResultRequest should match Figure 5", 
                normalizedSpec, normalizedGenerated);
    }
    
    /**
     * Test readResultResponse - matches Figure 6 from 
     * https://www.imsglobal.org/spec/lti-bo/v1p1
     * and Section 6.1.2 from https://www.imsglobal.org/specs/ltiv1p1p1/implementation-guide
     */
    @Test
    public void testReadResultResponse() throws Exception {
        // XML from Figure 6 of the spec
        ...
        assertEquals("readResultResponse should match Figure 6", 
                normalizedSpec, normalizedGenerated);
    }
    
    /**
     * Test deleteResultRequest - matches Figure 7 from 
     * https://www.imsglobal.org/spec/lti-bo/v1p1
     * and Section 6.1.3 from https://www.imsglobal.org/specs/ltiv1p1p1/implementation-guide
     */
    @Test
    public void testDeleteResultRequest() throws Exception {
        // XML from Figure 7 of the spec
        ...
        String normalizedSpec = normalizeToJacksonStyle(specXml);
        String normalizedGenerated = normalizeGeneratedXml(generatedXml);
        assertEquals("deleteResultRequest should match Figure 7", 
                normalizedSpec, normalizedGenerated);
    }
    
    /**
     * Test deleteResultResponse - matches Figure 8 from 
     * https://www.imsglobal.org/spec/lti-bo/v1p1
     * and Section 6.1.3 from https://www.imsglobal.org/specs/ltiv1p1p1/implementation-guide
     */
    @Test
    public void testDeleteResultResponse() throws Exception {
        // XML from Figure 8 of the spec
        String specXml = specXml_6_1_3_deleteResultResponse;
        
        // Build using POXResponseBuilder
        // Note: Figure 8 doesn't include a description field, so we don't set one
        String generatedXml = POXResponseBuilder.create()
        ...
        assertEquals("deleteResultResponse should match Figure 8", 
                normalizedSpec, normalizedGenerated);
    }
     ...
     * from Figure 3 of https://www.imsglobal.org/spec/lti-bo/v1p1
     */
    @Test
     ...
     * from Figure 4 of https://www.imsglobal.org/spec/lti-bo/v1p1
     * Note: v1.1.1 spec includes empty codeMinor element, which Jackson will parse as null
     */
    @Test
    public void testParseReplaceResultResponse() throws Exception {
        ...
        // Empty codeMinor in XML will be parsed as POXCodeMinor with null fields (not null itself)
        // This is acceptable - an empty codeMinor element means "no minor code"
        if (statusInfo.getCodeMinor() != null) {
            assertNull("CodeMinor fields should be null for empty element", statusInfo.getCodeMinor().getCodeMinorFields());
        }
     ...
     * from Figure 5 of https://www.imsglobal.org/spec/lti-bo/v1p1
     */
    @Test
     ...
     * from Figure 6 of https://www.imsglobal.org/spec/lti-bo/v1p1
     * Note: v1.1.1 spec includes empty codeMinor element, which Jackson will parse as null
     */
    @Test
     ...
     * from Figure 7 of https://www.imsglobal.org/spec/lti-bo/v1p1
     */
    @Test
     ...
     * from Figure 8 of https://www.imsglobal.org/spec/lti-bo/v1p1
     * Note: v1.1.1 spec includes empty codeMinor element, which Jackson will parse as null
     */
    @Test
