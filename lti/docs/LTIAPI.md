...
permissions, issuing bearer tokens, and exposing launch-time substitution variables so tools can
...
`lti_tool_permissions` table and enforced when tokens are issued. Please note that Sakai has,
historically, used the term "function" to represent a Sakai authz permission such as "content.read".
This document tries to stick with the term "permission" as it is more widely understood.

```mermaid
flowchart LR
  Admin[Admin: API Access UI] --> DB[(lti_tool_permissions)]
...
- Lists all **registered Sakai security permissions** from `FunctionManager.getRegisteredFunctions()`.
- Checked permissions are persisted in `lti_tool_permissions` (`tool_id`, `permission`).
- Unregistered permissions submitted from the UI are ignored.
- **Clear all** unchecks every permission on the form; **Clear all and save** persists an empty grant set.

## OAuth scopes and permissions

Tools request **OAuth scopes** at the token endpoint. Sakai maps each granted **permission** to a
scope:

| Permission (DB / checkbox) | OAuth scope (token request) |
|--------------------------|-----------------------------|
| `content.read` | `sakai.lti.api.content.read` |
| `gradebook.write` | `sakai.lti.api.gradebook.write` |

Mapping helpers: `SakaiAccessToken.permissionToLtiApiScope()` / `ltiApiScopeToPermission()`.
...
3. For each `sakai.lti.api.*` scope, grant only if the permission is in `lti_tool_permissions`.
...
Tools can read granted scopes from **custom parameters** using substitution
variables (set in the tool’s **Custom Parameters** field).

| Substitution variable | Value |
|----------------------|--------|
| `Sakai.scopes.available` | Space-separated OAuth scopes for permissions granted in API Access |
...
| `lti.directapi.enabled` | `true` | Allow LTI Bearer tokens on Entity Broker (`/direct`); when false, Bearer requests get 403 without validation |
...
SAT validation uses the same `SakaiAccessTokenService.validateToken()` as webapi. Scope-to-permission enforcement on
...
6. Set `lti.webapi.enabled=false` or `lti.directapi.enabled=false` → matching probe returns 403 without
